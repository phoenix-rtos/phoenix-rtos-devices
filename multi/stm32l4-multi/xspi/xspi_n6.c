/*
 * Phoenix-RTOS
 *
 * STM32 XSPI Flash driver
 *
 * Copyright 2026 Phoenix Systems
 * Author: Jacek Maksymowicz
 *
 * This file is part of Phoenix-RTOS.
 *
 * %LICENSE%
 */

#include "xspi_n6.h"

#include <stdio.h>
#include <errno.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>
#include <sys/threads.h>
#include <sys/interrupt.h>
#include <sys/mman.h>
#include <sys/time.h>
#include <libmulti/libdma.h>

#include "../common.h"
#include "../stm32l4-multi.h"
#include "../ext_flash.h"
#include "mce.h"

#include <board_config.h>
#include <phoenix/arch/armv8m/stm32/flash_chip_setup.h>


/* clang-format off */
#define TRACE(fmt, ...) do { if (0) { printf("%s:%d: " fmt "\n", __func__, __LINE__, ##__VA_ARGS__); } } while (0)
#define LOG_ERROR(str, ...) do { fprintf(stderr, "%s:%d error: " str "\n", __func__, __LINE__, ##__VA_ARGS__); } while (0)
/* clang-format on */


#define XSPI1_REG_SIZE (256 * 1024 * 1024)
#define XSPI2_REG_SIZE (256 * 1024 * 1024)
#define XSPI3_REG_SIZE (256 * 1024 * 1024)

#define REGION_NOT_ENCRYPTED mce_regcount

enum {
	ipclk_sel_hclk5 = 0x0,
	ipclk_sel_per_ck = 0x1,
	ipclk_sel_ic3_ck = 0x2,
	ipclk_sel_ic4_ck = 0x3,
};


#define XSPI1_POS 0
#define XSPI2_POS (XSPI1_POS + XSPI1_STORAGE)
#define XSPI3_POS (XSPI2_POS + XSPI2_STORAGE)

#define N_XSPI_ACTIVE (XSPI1_STORAGE + XSPI2_STORAGE + XSPI3_STORAGE)

#define N_XSPI_CONTROLLERS 3

static unsigned int xspi_ctrlPos[N_XSPI_CONTROLLERS] = {
	(XSPI1_STORAGE != 0) ? XSPI1_POS : N_XSPI_ACTIVE,
	(XSPI2_STORAGE != 0) ? XSPI2_POS : N_XSPI_ACTIVE,
	(XSPI3_STORAGE != 0) ? XSPI3_POS : N_XSPI_ACTIVE,
};


static const xspi_ctrlParams_t xspi_ctrlParams[N_XSPI_ACTIVE] = {
#if XSPI1_STORAGE
	[XSPI1_POS] = {
		.start = XSPI1_REG_BASE,
		.size = XSPI1_REG_SIZE,
		.clksel = { .sel = pctl_ipclk_xspi1sel, .val = ipclk_sel_hclk5 },
		.divider_slow = 6,
		.divider = XSPI1_CLOCK_DIV,
		.dev = pctl_xspi1,
		.irq = xspi1_irq,
		.ctrl = XSPI1_BASE,
		.resetPin = { pctl_gpioo, 5 },
		.spiPort = XSPIM_PORT1,
		.chipSelect = XSPI_CHIPSELECT_NCS1,
		.isHyperbus = XSPI1_IS_HYPERBUS,
		.mceDev = pctl_mce1,
	},
#endif
#if XSPI2_STORAGE
	[XSPI2_POS] = {
		.start = XSPI2_REG_BASE,
		.size = XSPI2_REG_SIZE,
		.clksel = { .sel = pctl_ipclk_xspi2sel, .val = ipclk_sel_hclk5 },
		.divider_slow = 6,
		.divider = XSPI2_CLOCK_DIV,
		.dev = pctl_xspi2,
		.irq = xspi2_irq,
		.ctrl = XSPI2_BASE,
		.resetPin = { pctl_gpion, 12 },
		.spiPort = XSPIM_PORT2,
		.chipSelect = XSPI_CHIPSELECT_NCS1,
		.isHyperbus = XSPI2_IS_HYPERBUS,
		.mceDev = pctl_mce2,
	},
#endif
#if XSPI3_STORAGE
	[XSPI3_POS] = {
		.start = XSPI3_REG_BASE,
		.size = XSPI3_REG_SIZE,
		.clksel = { .sel = pctl_ipclk_xspi3sel, .val = ipclk_sel_hclk5 },
		.divider_slow = 6,
		.divider = XSPI3_CLOCK_DIV,
		.dev = pctl_xspi3,
		.irq = xspi3_irq,
		.ctrl = XSPI3_BASE,
		.resetPin = { -1, -1 },
		.spiPort = XSPIM_PORT2,
		.chipSelect = XSPI_CHIPSELECT_NCS1,
		.isHyperbus = XSPI3_IS_HYPERBUS,
		.mceDev = pctl_mce3,
	},
#endif
};


typedef struct {
	uint32_t ccr;
	uint32_t tcr;
	uint32_t ir;
} xspi_cmdRegs_t;


typedef struct {
	xspi_cmdRegs_t reg;
	uint32_t addr;
	uint32_t dataLen;
	uint8_t isRead;
} flash_opDefinition_t;


static struct {
	struct xspi_ctrlSetup {
		bool configured;
		handle_t mutex;
		handle_t irq;
		handle_t irq_mutex;
		handle_t irq_cond;
		const struct libdma_per *dma;
		struct xspi_mem {
			char name[32];
			uint8_t jedecID[6];
			uint8_t log_chipSize;
			uint8_t log_eraseSize;
			uint8_t log_pageSize;
			uint32_t eraseTimeoutMs;
			uint32_t chipEraseTimeoutMs;
			uint32_t writePageTimeoutUs;
			xspi_cmdRegs_t read;
			xspi_cmdRegs_t write;
			xspi_cmdRegs_t erase;
			flash_opDefinition_t chipErase;
			flash_opDefinition_t writeEnable;
			flash_opDefinition_t writeDisable;
			flash_opDefinition_t readStatus;
		} mem;
		mce_regionInfo_t region[mce_regcount];
	} ctrl[N_XSPI_ACTIVE];
} xspi_common;


static inline int getDevClk(int dev)
{
	platformctl_t pctl;
	pctl.action = pctl_get;
	pctl.type = pctl_devclk;
	pctl.devclk.dev = dev;
	if (platformctl(&pctl) < 0) {
		return 0;
	}

	return pctl.devclk.state;
}


/* Note: currently libphoenix doesn't implement pthread_rwlock*, so for now we use regular mutexes
 * to implement mutual exclusion.
 * If any additional actions need to be taken when switching controller modes, they can be
 * added to this function (maybe some way to prevent accesses to Flash?) */
static int xspi_rwlock_acquire(unsigned int idx, bool doWrite)
{
	(void)doWrite;
	return mutexLock(xspi_common.ctrl[idx].mutex);
}


static void xspi_rwlock_release(unsigned int idx)
{
	mutexUnlock(xspi_common.ctrl[idx].mutex);
}


static inline unsigned int xspi_lookupDevIdx(unsigned int devNum)
{
	if (devNum >= N_XSPI_CONTROLLERS) {
		return N_XSPI_ACTIVE;
	}

	unsigned int idx = xspi_ctrlPos[devNum];
	if (idx >= N_XSPI_ACTIVE) {
		return idx;
	}

	return xspi_common.ctrl[idx].configured ? idx : N_XSPI_ACTIVE;
}


/* -1 - address range crosses a region boundary
 * [0 .. (XSPI_MCE_REGIONS - 1)] - number of encrypted region
 *  REGION_NOT_ENCRYPTED - not encrypted
 */
static int xspi_getRegion(unsigned int idx, uint32_t off, size_t end)
{
	for (int i = 0; i < mce_regcount; i++) {
		mce_regionInfo_t *r = &xspi_common.ctrl[idx].region[i];
		if (!r->enabled) {
			continue;
		}

		if (((off < r->start) && (end > r->start)) || ((off < r->end) && (end > r->end))) {
			return -1;
		}

		if ((r->start <= off) && (end <= r->end)) {
			return i;
		}
	}

	return REGION_NOT_ENCRYPTED;
}


static int xspi_validateAndGetRegion(unsigned int idx, uint32_t off, size_t size)
{
	size_t fsize;
	size_t end = off + size;
	int region;

	if (end < off) {
		/* Integer overflow has occurred */
		return -EINVAL;
	}

	fsize = 1UL << xspi_common.ctrl[idx].mem.log_chipSize;
	if (!((off < fsize) && ((off + size) <= fsize))) {
		return -EINVAL;
	}

	region = xspi_getRegion(idx, off, end);
	if (region < 0) {
		return -EINVAL;
	}

	return region;
}


static inline void xspi_waitBusy(unsigned int idx)
{
	while ((*(xspi_ctrlParams[idx].ctrl + xspi_sr) & XSPI_SR_BUSY) != 0) {
		/* Wait for controller to become ready */
	}
}


/* Transfer data in indirect read or write mode using the FIFO.
 * `data`: output (if isRead == 0) or input (if isRead != 0) data buffer
 * `len`: length of data to transfer
 * `isRead`: == 0 if this is a write operation, != 0 otherwise */
static int xspi_transferFifo(unsigned int idx, uint8_t *data, size_t len, uint8_t isRead)
{
	const xspi_ctrlParams_t *p = &xspi_ctrlParams[idx];
	size_t i = 0, j = 0;
	uint32_t status;
	time_t deadline;

	while (i < len) {
		status = *(p->ctrl + xspi_sr);
		/* Optimization - check status first, then calculate deadline.
		 * Because XSPI is very fast we can get bytes in FIFO within a few CPU clock cycles. */
		if ((status & (XSPI_SR_FTF | XSPI_SR_TCF)) == 0) {
			/* Timeout is very generous - at least 1 ms per byte. Device should never time out
			 * unless there is a hardware failure. */
			time_t now;
			gettime(&now, NULL);
			deadline = now + 1000;
			do {
				gettime(&now, NULL);
				status = *(p->ctrl + xspi_sr);
				if (now > deadline) {
					*(p->ctrl + xspi_cr) |= (1 << 1); /* Abort operation in progress */
					xspi_waitBusy(idx);
					return -ETIME;
				}
			} while ((status & (XSPI_SR_FTF | XSPI_SR_TCF)) == 0);
		}

		if (isRead != 0) {
			j = XSPI_FIFO_SIZE - ((status >> 8) & 0x7f);
			for (; j < XSPI_FIFO_SIZE && (i < len); j++, i++) {
				/* This controller allows byte and halfword reads from the XSPI_DR register */
				data[i] = *(volatile uint8_t *)(p->ctrl + xspi_dr);
			}
		}
		else {
			j = (status >> 8) & 0x7f;
			/* In indirect write mode, writing data triggers the operation */
			for (; j < XSPI_FIFO_SIZE && (i < len); j++, i++) {
				/* This controller allows byte and halfword writes to the XSPI_DR register */
				*(volatile uint8_t *)(p->ctrl + xspi_dr) = data[i];
			}
		}
	}

	while ((*(p->ctrl + xspi_sr) & XSPI_SR_TCF) == 0) {
		/* Wait for transfer completion */
	}

	*(p->ctrl + xspi_fcr) = XSPI_SR_TCF;
	return EOK;
}


static inline int xspi_opHasAddr(const flash_opDefinition_t *opDef)
{
	return ((opDef->reg.ccr >> 8) & 0x7) != 0;
}


static uint32_t xspi_changeCtrlMode(unsigned int idx, uint32_t new_mode, int doMemWriting)
{
	const xspi_ctrlParams_t *p = &xspi_ctrlParams[idx];
	struct xspi_mem *mem = &xspi_common.ctrl[idx].mem;
	uint32_t prev_mode, v;

	v = *(p->ctrl + xspi_cr);
	prev_mode = (v & XSPI_CR_MODE_MASK);
	if (prev_mode == new_mode) {
		return prev_mode;
	}

	if ((prev_mode == XSPI_CR_MODE_MEMORY) || (prev_mode == XSPI_CR_MODE_AUTOPOLL)) {
		dataBarier();
		*(p->ctrl + xspi_cr) = v | (1 << 1); /* Abort operation in progress */
		xspi_waitBusy(idx);
	}

	v &= ~XSPI_CR_MODE_MASK;
	if (new_mode == XSPI_CR_MODE_MEMORY) {
		*(p->ctrl + xspi_cr) = v;
		dataBarier();
		xspi_waitBusy(idx);

		*(p->ctrl + xspi_ccr) = mem->read.ccr;
		*(p->ctrl + xspi_tcr) = mem->read.tcr;
		*(p->ctrl + xspi_ir) = mem->read.ir;
		/* Clear prefetch and timeout bits */
		v &= ~(XSPI_CR_NOPREF_AXI | XSPI_CR_NOPREF | XSPI_CR_TCEN);
		if (doMemWriting != 0) {
			*(p->ctrl + xspi_wccr) = mem->write.ccr;
			*(p->ctrl + xspi_wtcr) = mem->write.tcr;
			*(p->ctrl + xspi_wir) = mem->write.ir;
			/* Leave prefetch and timeout bits cleared to enable prefetch and disable timeout */
		}
		else {
			/* Disable writing. Attempting to write will cause an exception. */
			*(p->ctrl + xspi_wccr) = 0;
			v |= XSPI_DEFAULT_PREFETCH | XSPI_DEFAULT_TIMEOUT;
		}

		dataBarier();
	}

	v |= new_mode;
	*(p->ctrl + xspi_cr) = v;
	xspi_waitBusy(idx);

	/* Clear any flags that may have been set (e.g. in autopoll or memory-mapped mode) */
	*(p->ctrl + xspi_fcr) = XSPI_SR_TCF | XSPI_SR_SMF;
	return prev_mode;
}


static int xspi_performOp(unsigned int idx, const flash_opDefinition_t *opDef, unsigned char *data)
{
	const xspi_ctrlParams_t *p = &xspi_ctrlParams[idx];
	xspi_changeCtrlMode(idx, (opDef->isRead != 0) ? XSPI_CR_MODE_IREAD : XSPI_CR_MODE_IWRITE, 0);
	if (opDef->dataLen != 0) {
		*(p->ctrl + xspi_dlr) = opDef->dataLen - 1;
	}

	*(p->ctrl + xspi_ccr) = opDef->reg.ccr;
	*(p->ctrl + xspi_tcr) = opDef->reg.tcr;
	/* If indirect read mode and no address, this write triggers the operation */
	*(p->ctrl + xspi_ir) = opDef->reg.ir;
	if (xspi_opHasAddr(opDef)) {
		/* If indirect read mode with address, this write triggers the operation */
		*(p->ctrl + xspi_ar) = opDef->addr;
	}

	return xspi_transferFifo(idx, data, opDef->dataLen, opDef->isRead);
}


static int xspi_waitForWriteCompletion(unsigned int idx, const flash_opDefinition_t *opDef, time_t timeout_us)
{
	int ret = 0;
	const xspi_ctrlParams_t *p = &xspi_ctrlParams[idx];
	struct xspi_ctrlSetup *s = &xspi_common.ctrl[idx];

	xspi_changeCtrlMode(idx, XSPI_CR_MODE_AUTOPOLL, 0);
	*(p->ctrl + xspi_psmkr) = 0x01; /* Check bit 0 */
	*(p->ctrl + xspi_psmar) = 0x00; /* Check until it's cleared */
	*(p->ctrl + xspi_pir) = 0x10;
	*(p->ctrl + xspi_dlr) = opDef->dataLen - 1;
	*(p->ctrl + xspi_ccr) = opDef->reg.ccr;
	*(p->ctrl + xspi_tcr) = opDef->reg.tcr;
	/* If command has no address, writing XSPI_IR starts the operation */
	*(p->ctrl + xspi_ir) = opDef->reg.ir;
	if (xspi_opHasAddr(opDef)) {
		/* If command has address, writing XSPI_AR starts the operation */
		*(p->ctrl + xspi_ar) = opDef->addr;
	}

	mutexLock(s->irq_mutex);
	*(p->ctrl + xspi_cr) |= XSPI_CR_SMIE;
	time_t now, end;
	gettime(&now, NULL);
	end = now + timeout_us;
	while (1) {
		condWait(s->irq_cond, s->irq_mutex, timeout_us);
		if ((*(p->ctrl + xspi_sr) & XSPI_SR_SMF) != 0) {
			break;
		}

		gettime(&now, NULL);
		if (end <= now) {
			*(p->ctrl + xspi_cr) |= (1 << 1); /* Abort auto-polling */
			ret = -ETIME;
			break;
		}
	}

	mutexUnlock(s->irq_mutex);
	*(p->ctrl + xspi_fcr) = XSPI_SR_SMF;
	return ret;
}


static int xspi_writeEnable(unsigned int idx, bool enable)
{
	/* Depending on command status may be 1 or 2 bytes long.
	 * We only care about the first byte, but need to allocate a large enough buffer. */
	uint8_t status[2];
	const flash_opDefinition_t *opDef =
			enable ?
			&xspi_common.ctrl[idx].mem.writeEnable :
			&xspi_common.ctrl[idx].mem.writeDisable;
	unsigned retries = 10;
	int ret;
	enable = (enable != 0) ? 1 : 0;

	/* Set flag and verify until it's set - required according to Macronix datasheet */
	do {
		if (retries == 0) {
			return -EIO;
		}

		retries--;
		ret = xspi_performOp(idx, opDef, NULL);
		if (ret < 0) {
			return ret;
		}

		ret = xspi_performOp(idx, &xspi_common.ctrl[idx].mem.readStatus, status);
		if (ret < 0) {
			return ret;
		}
	} while (((status[0] >> 1) & 1) != enable);

	return EOK;
}


/* Perform a write-like operation (program or erase) */
static ssize_t xspi_performWriteOp(
		unsigned int idx,
		const flash_opDefinition_t *opDef,
		const void *data,
		time_t timeout_us)
{
	int ret;
	if (xspi_writeEnable(idx, 1) < 0) {
		return -EIO;
	}

	ret = xspi_performOp(idx, opDef, (void *)data);
	if (ret < 0) {
		return ret;
	}

	return xspi_waitForWriteCompletion(idx, &xspi_common.ctrl[idx].mem.readStatus, timeout_us);
}


static int xspi_invalCache(const void *addr, size_t sz)
{
	platformctl_t pctl;
	pctl.action = pctl_set;
	pctl.type = pctl_invalDCache;
	pctl.opDCache.addr = (void *)addr;
	pctl.opDCache.sz = sz;
	return platformctl(&pctl);
}


static int xspi_dmaWrite(const struct libdma_per *per, void *dst, const void *src, size_t len)
{
	int ret = 0;
	/* Here we optimize the transfer - we pick a maximum data width for both source and destination
	 * depending on pointer alignment and transfer length (up to 8 bytes).
	 * Transfer length must be a multiple of burst length and AXI supports max 16 data units.
	 * If MCE is in block mode we assume that destination and length are multiples of 16
	 * (it should have been verified previously), so logDat_dst == 3, burst_dst == 2.
	 */
	size_t logDat_dst = min(__builtin_ctz((addr_t)dst | len), 3);
	size_t logDat_src = min(__builtin_ctz((addr_t)src | len), 3);
	size_t logBurst = min(__builtin_ctz(len), 4);
	/* Calculate how many data units are to a burst. */
	uint8_t burst_src = 1U << (logBurst - logDat_src);
	uint8_t burst_dst = 1U << (logBurst - logDat_dst);

	libdma_peripheral_config_t cfg = {
		.addr = dst,
		.elSize_log = logDat_dst,
		.burstSize = burst_dst,
		.increment = 1,
	};

	libdma_transfer_buffer_t bufs[2];
	bufs[0] = (libdma_transfer_buffer_t) {
		.buf = (void *)src,
		.bufSize = len,
		.elSize_log = logDat_src,
		.burstSize = burst_src,
		.increment = 1,
		.isCached = 1,
		/* Packed mode ensures the same byte sequence will result regardless of source/destination element size. */
		.transform = LIBXPDMA_TRANSFORM_PACK,
	};
	size_t nBufs = 1;

	/* If ending is misaligned, handle the write-end bug */
	void *end = dst + len;
	uint32_t misalign = 4 - ((addr_t)end % 4);
	uint8_t misalignData[4];
	if (misalign < 4) {
		/* This read must occur before any writes */
		memcpy(misalignData, end, misalign);
		bufs[1] = (libdma_transfer_buffer_t) {
			.buf = (void *)misalignData,
			.bufSize = misalign,
			.elSize_log = 0,
			.burstSize = 1,
			.increment = 1,
			.isCached = 1,
			.transform = LIBXPDMA_TRANSFORM_PACK,
		};
		nBufs = 2;
	}

	ret = (ret < 0) ? ret : libxpdma_configurePeripheral(per, dma_mem2per, &cfg);
	ret = (ret < 0) ? ret : libxpdma_configureMemory(per, dma_mem2per, 0, bufs, nBufs);
	/* Because XSPI transfer is very fast, it's faster to wait through polling only
	 * (no overhead caused by syscalls/ISR/scheduler etc.) */
	ret = (ret < 0) ? ret : libxpdma_startTransferPollingOnly(per, dma_mem2per);
	ret = (ret < 0) ? ret : libxpdma_waitPoll(per, dma_mem2per, 0);
	ret = (ret < 0) ? ret : xspi_invalCache(dst, len);
	return ret;
}


static int xspi_isr(unsigned int irq, void *arg)
{
	unsigned int idx = (unsigned int)arg;
	uint32_t status = *(xspi_ctrlParams[idx].ctrl + xspi_sr);
	status &= (XSPI_SR_TOF | XSPI_SR_SMF | XSPI_SR_FTF | XSPI_SR_TCF | XSPI_SR_TEF);
	*(xspi_ctrlParams[idx].ctrl + xspi_cr) &= ~(status << 16);
	return 1;
}


static void xspi_chipSetup_copyCmdRegs(xspi_cmdRegs_t *mem, flashcs_regs_v1_t *cs)
{
	mem->ccr = (uint32_t)cs->ccr;
	mem->tcr = (uint32_t)cs->tcr;
	mem->ir = (uint32_t)cs->ir;
}


static void xspi_chipSetup_deserialize(unsigned int idx, flashcs_v1_basic_t *cs)
{
	struct xspi_mem *mem = &xspi_common.ctrl[idx].mem;
	mem->log_chipSize = (uint8_t)cs->log_chipSize;
	mem->log_eraseSize = (uint8_t)cs->log_eraseSize;
	mem->log_pageSize = (uint8_t)cs->log_pageSize;
	mem->eraseTimeoutMs = (uint32_t)cs->eraseTimeoutMs;
	mem->chipEraseTimeoutMs = (uint32_t)cs->chipEraseTimeoutMs;
	mem->writePageTimeoutUs = (uint32_t)cs->writePageTimeoutUs;
	strncpy(mem->name, cs->name, min(sizeof(mem->name), sizeof(cs->name)));
	mem->name[sizeof(mem->name) - 1] = '\0';
	memcpy(mem->jedecID, cs->jedecID, sizeof(mem->jedecID));
	xspi_chipSetup_copyCmdRegs(&mem->read, &cs->read);
	xspi_chipSetup_copyCmdRegs(&mem->write, &cs->write);
	xspi_chipSetup_copyCmdRegs(&mem->erase, &cs->erase);

	xspi_chipSetup_copyCmdRegs(&mem->chipErase.reg, &cs->chipErase);
	mem->chipErase.addr = 0;
	mem->chipErase.dataLen = 0;
	mem->chipErase.isRead = 0;

	xspi_chipSetup_copyCmdRegs(&mem->writeEnable.reg, &cs->writeEnable);
	mem->writeEnable.addr = 0;
	mem->writeEnable.dataLen = 0;
	mem->writeEnable.isRead = 0;

	xspi_chipSetup_copyCmdRegs(&mem->writeDisable.reg, &cs->writeDisable);
	mem->writeDisable.addr = 0;
	mem->writeDisable.dataLen = 0;
	mem->writeDisable.isRead = 0;

	xspi_chipSetup_copyCmdRegs(&mem->readStatus.reg, &cs->readStatus);
	mem->readStatus.addr = (uint32_t)cs->readStatus_addr;
	mem->readStatus.dataLen = (uint8_t)cs->readStatus_dataLen;
	mem->readStatus.isRead = 1;
}


static int xspi_chipSetup_lookupSyspage(const char *name, void **data, size_t *size)
{
	int progsz = syspageprog(NULL, -1);
	if (progsz < 0) {
		return -ENOENT;
	}

	for (int i = 0; i < progsz; i++) {
		syspageprog_t prog;
		if (syspageprog(&prog, i) != 0) {
			continue;
		}

		if (strcmp(prog.name, name) == 0) {
			*data = (void *)prog.addr;
			*size = prog.size;
			return EOK;
		}
	}

	return -ENOENT;
}


static int xspi_chipSetup_readSyspage(unsigned int idx)
{
	char memconfig_path[32];
	unsigned int portNum;
	switch (xspi_ctrlParams[idx].spiPort) {
		case XSPIM_PORT1: portNum = 1; break;
		case XSPIM_PORT2: portNum = 2; break;
		default: return -EINVAL;
	}

	int ret = snprintf(memconfig_path, sizeof(memconfig_path), FLASHCS_FORMAT, portNum);
	if (ret >= sizeof(memconfig_path)) {
		return -ENAMETOOLONG;
	}

	void *data;
	size_t size;
	ret = xspi_chipSetup_lookupSyspage(memconfig_path, &data, &size);
	if (ret < 0) {
		return ret;
	}

	flashcs_v1_header_t *hdr = data;
	flashcs_v1_basic_t *cs = data + sizeof(*hdr);
	size_t expectedSize = sizeof(*hdr) + sizeof(*cs);
	if (size < expectedSize) {
		return -ENOTSUP;
	}

	if ((hdr->version != FLASHCS_VER_1) || ((hdr->features & FLASHCS_FEAT_BASIC) == 0)) {
		return -ENOTSUP;
	}

	xspi_chipSetup_deserialize(idx, cs);
	return EOK;
}


static int xspi_initOne(unsigned int idx)
{
	const xspi_ctrlParams_t *p = &xspi_ctrlParams[idx];
	struct xspi_ctrlSetup *s = &xspi_common.ctrl[idx];
	if (s->configured) {
		TRACE("already initialized");
		return -EALREADY;
	}

	/* Currently we don't have support for Hyperbus Flash devices */
	if (p->isHyperbus != 0) {
		return -ENOTSUP;
	}

	if (getDevClk(p->dev) == 0) {
		/* Device disabled; sometime we may add support for initialization from scratch */
		TRACE("not initialized");
		return -EIO;
	}

	uint32_t crValue = *(p->ctrl + xspi_cr);
	if (((crValue & 1) == 0) || ((crValue & XSPI_CR_MODE_MASK) != XSPI_CR_MODE_MEMORY)) {
		/* Device not configured; sometime we may add support for initialization from scratch */
		TRACE("not initialized");
		return -EIO;
	}

	for (int i = 0; i < mce_regcount; i++) {
		if (mce_getRegionSetup(xspi_ctrlParams[idx].mceDev, i, &xspi_common.ctrl[idx].region[i]) < 0) {
			xspi_common.ctrl[idx].region[i].enabled = false;
		}
	}

	/* Controller is already initialized and in memory-mapped mode. */
	int ret = xspi_chipSetup_readSyspage(idx);
	if (ret < 0) {
		LOG_ERROR("XSPI configuration read error %d", ret);
		return ret;
	}

	if (s->mem.log_chipSize > 31) {
		LOG_ERROR("Chip has size over 2 GB - full capacity not supported");
		s->mem.log_chipSize = 31;
	}

	if ((1UL << s->mem.log_chipSize) > p->size) {
		LOG_ERROR("Chip has size larger than memory map - full capacity not supported");
		s->mem.log_chipSize = __builtin_ctz(p->size);
	}

	/* Perform some sanity checks on the info that was read */
	if ((s->mem.log_eraseSize > s->mem.log_chipSize) || (s->mem.log_pageSize > s->mem.log_chipSize)) {
		return -EINVAL;
	}

	const struct libdma_per *per;
	ret = libxpdma_acquirePeripheral(dma_memTransfer, memTransfer_perIsDst, LIBXPDMA_ACQUIRE_2D_MEM2PER, &per);
	if (ret < 0) {
		TRACE("DMA acquire %d", ret);
		return ret;
	}

	ret = libxpdma_configureChannel(per, dma_mem2per, dma_priorityVeryHigh, NULL);
	if (ret < 0) {
		TRACE("DMA configure %d", ret);
		return ret;
	}

	s->dma = per;

	if (mutexCreate(&s->mutex) < 0) {
		return -ENOMEM;
	}

	if (mutexCreate(&s->irq_mutex) < 0) {
		resourceDestroy(s->mutex);
		return -ENOMEM;
	}

	if (condCreate(&s->irq_cond) < 0) {
		resourceDestroy(s->mutex);
		resourceDestroy(s->irq_mutex);
		return -ENOMEM;
	}

	if (interrupt(p->irq, xspi_isr, (void *)idx, s->irq_cond, &s->irq) < 0) {
		resourceDestroy(s->mutex);
		resourceDestroy(s->irq_mutex);
		resourceDestroy(s->irq_cond);
		return -ENOMEM;
	}

	s->configured = true;
	return EOK;
}


int extFlash_fn_init(void)
{
	memset(&xspi_common, 0, sizeof(xspi_common));
	for (unsigned int devNum = 0; devNum < N_XSPI_CONTROLLERS; devNum++) {
		unsigned int idx = xspi_ctrlPos[devNum];
		if (idx >= N_XSPI_ACTIVE) {
			continue;
		}

		xspi_initOne(idx);
	}

	return EOK;
}


ssize_t extFlash_fn_read(unsigned int devNum, addr_t offs, void *buff, size_t len)
{
	unsigned int idx = xspi_lookupDevIdx(devNum);
	if (idx >= N_XSPI_ACTIVE) {
		return -ENODEV;
	}

	int region = xspi_validateAndGetRegion(idx, offs, len);
	if (region < 0) {
		return region;
	}

	if (len == 0) {
		return 0;
	}

	if (xspi_rwlock_acquire(idx, false) < 0) {
		return -EBUSY;
	}

	/* TODO: for large reads consider using mem-to-mem DMA transfer - it may free up CPU for other tasks */
	memcpy(buff, xspi_ctrlParams[idx].start + offs, len);
	xspi_rwlock_release(idx);
	return len;
}


static int xspi_write_page(unsigned int idx, void *dest, const void *src, size_t *i, size_t n)
{
	struct xspi_mem *mem = &xspi_common.ctrl[idx].mem;
	int ret;
	ret = xspi_writeEnable(idx, 1);
	if (ret < 0) {
		return ret;
	}

	xspi_changeCtrlMode(idx, XSPI_CR_MODE_MEMORY, 1);
	ret = xspi_dmaWrite(xspi_common.ctrl[idx].dma, dest + *i, src + *i, n);
	if (ret < 0) {
		return ret;
	}

	ret = xspi_waitForWriteCompletion(idx, &mem->readStatus, mem->writePageTimeoutUs);
	if (ret < 0) {
		return ret;
	}

	*i += n;
	return 0;
}


static ssize_t xspi_write_internal(unsigned int idx, addr_t offs, const void *buff, size_t len)
{
	const xspi_ctrlParams_t *p = &xspi_ctrlParams[idx];
	/* Limit page size to 32 KB (DMA supports 65535 B max) */
	size_t page_size = 1UL << min(xspi_common.ctrl[idx].mem.log_pageSize, 15);

	size_t i = 0;
	int ret;

	if ((offs % page_size) != 0) {
		ret = xspi_write_page(idx, p->start + offs, buff, &i, min(page_size - (offs % page_size), len));
		if (ret < 0) {
			return ret;
		}
	}

	while ((len - i) >= page_size) {
		ret = xspi_write_page(idx, p->start + offs, buff, &i, page_size);
		if (ret < 0) {
			return ret;
		}
	}

	if ((len - i) > 0) {
		ret = xspi_write_page(idx, p->start + offs, buff, &i, len - i);
		if (ret < 0) {
			return ret;
		}
	}

	return (ssize_t)i;
}


ssize_t extFlash_fn_write(unsigned int devNum, addr_t offs, const void *buff, size_t len)
{
	unsigned int idx = xspi_lookupDevIdx(devNum);
	if (idx >= N_XSPI_ACTIVE) {
		return -ENODEV;
	}

	int region = xspi_validateAndGetRegion(idx, offs, len);
	if (region < 0) {
		return region;
	}

	if (region != REGION_NOT_ENCRYPTED) {
		size_t granularity = xspi_common.ctrl[idx].region[region].granularity;
		if (granularity > 1) {
			if (((offs % granularity) != 0) || ((len % granularity) != 0)) {
				return -EINVAL;
			}
		}
	}

	if (len == 0) {
		return 0;
	}

	if (xspi_rwlock_acquire(idx, true) < 0) {
		return -EBUSY;
	}

	uint32_t prev_mode = xspi_changeCtrlMode(idx, XSPI_CR_MODE_IWRITE, 0);
	ssize_t ret = xspi_write_internal(idx, offs, buff, len);
	xspi_changeCtrlMode(idx, prev_mode, 0);
	xspi_invalCache(xspi_ctrlParams[idx].start + offs, len);
	xspi_rwlock_release(idx);
	return ret;
}


static ssize_t xspi_erase_internal(unsigned int idx, addr_t offs, size_t len)
{
	flash_opDefinition_t op;
	size_t eraseSize = 1UL << xspi_common.ctrl[idx].mem.log_eraseSize;
	ssize_t len_ret = (ssize_t)len;
	op.reg = xspi_common.ctrl[idx].mem.erase;
	op.isRead = 0;
	op.dataLen = 0;
	for (; len != 0; offs += eraseSize, len -= eraseSize) {
		op.addr = offs;
		int ret = xspi_performWriteOp(
				idx,
				&op,
				NULL,
				(time_t)xspi_common.ctrl[idx].mem.eraseTimeoutMs * 1000);
		if (ret < 0) {
			return ret;
		}
	}

	return (ssize_t)len_ret;
}


ssize_t extFlash_fn_erase(unsigned int devNum, addr_t offs, size_t len)
{
	unsigned int idx = xspi_lookupDevIdx(devNum);
	if (idx >= N_XSPI_ACTIVE) {
		return -ENODEV;
	}

	int region = xspi_validateAndGetRegion(idx, offs, len);
	if (region < 0) {
		return region;
	}

	size_t eraseSize = 1UL << xspi_common.ctrl[idx].mem.log_eraseSize;
	if (((offs % eraseSize) != 0) || ((len % eraseSize) != 0)) {
		return -EINVAL;
	}

	if (xspi_rwlock_acquire(idx, true) < 0) {
		return -EBUSY;
	}

	uint32_t prev_mode = xspi_changeCtrlMode(idx, XSPI_CR_MODE_IWRITE, 0);
	ssize_t ret = xspi_erase_internal(idx, offs, len);
	xspi_changeCtrlMode(idx, prev_mode, 0);
	xspi_invalCache(xspi_ctrlParams[idx].start + offs, len);
	xspi_rwlock_release(idx);
	return ret;
}


int extFlash_fn_chipErase(unsigned int devNum)
{
	unsigned int idx = xspi_lookupDevIdx(devNum);
	if (idx >= N_XSPI_ACTIVE) {
		return -ENODEV;
	}

	if (xspi_rwlock_acquire(idx, true) < 0) {
		return -EBUSY;
	}

	uint32_t prev_mode = xspi_changeCtrlMode(idx, XSPI_CR_MODE_IWRITE, 0);
	int ret = xspi_performWriteOp(
			idx,
			&xspi_common.ctrl[idx].mem.chipErase,
			NULL,
			(time_t)xspi_common.ctrl[idx].mem.chipEraseTimeoutMs * 1000);
	xspi_changeCtrlMode(idx, prev_mode, 0);
	xspi_invalCache(xspi_ctrlParams[idx].start, (1UL << xspi_common.ctrl[idx].mem.log_chipSize));
	xspi_rwlock_release(idx);
	return ret;
}


int extFlash_fn_sync(unsigned int devNum)
{
	unsigned int idx = xspi_lookupDevIdx(devNum);
	if (idx >= N_XSPI_ACTIVE) {
		return -ENODEV;
	}

	if (xspi_rwlock_acquire(idx, false) < 0) {
		return -EBUSY;
	}

	xspi_invalCache(xspi_ctrlParams[idx].start, (1UL << xspi_common.ctrl[idx].mem.log_chipSize));
	xspi_rwlock_release(idx);
	return EOK;
}


int extFlash_fn_getInfo(unsigned int devNum, extFlashDef_t *info)
{
	TRACE("%u", devNum);
	info->nDevices = N_XSPI_CONTROLLERS;
	unsigned int idx = xspi_lookupDevIdx(devNum);
	if (idx >= N_XSPI_ACTIVE) {
		return -ENODEV;
	}

	/* No locking required - there are no interactions with memory. */
	struct xspi_ctrlSetup *s = &xspi_common.ctrl[idx];
	strncpy(info->name, s->mem.name, sizeof(info->name));
	info->name[sizeof(info->name) - 1] = '\0';
	info->eraseSize = (1UL << s->mem.log_eraseSize);
	/* We report the maximum granularity of all regions as the write size.
	 * This is not optimal, but safe. */
	unsigned int maxGranularity = 1; /* writeSize must be at least 1 */
	for (int i = 0; i < mce_regcount; i++) {
		if (s->region[i].enabled) {
			maxGranularity = max(s->region[i].granularity, maxGranularity);
		}
	}

	info->writeSize = maxGranularity;
	info->pageSize = (1UL << s->mem.log_pageSize);
	info->totalSize = (1UL << s->mem.log_chipSize);
	return EOK;
}
