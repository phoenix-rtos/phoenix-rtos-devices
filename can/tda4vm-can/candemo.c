...
struct mcan_txBuff txBuff[MAX_FRAME_CNT];
struct mcan_rxMessage rxBuff[MAX_FRAME_CNT];
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "s:t:h")) != -1) {
		switch (opt) {
			case 's':
				...
				if (xferSize < 1 || xferSize > MAX_FRAME_CNT) {
	...
	if (init.canMode != MCU_MCAN_MODE_LOOPBACK) {
		if (gpio_initModule(MCAN_GPIO_MODULE) != EOK) {
			fprintf(stderr, "[MCAN DEMO] GPIO module initialization failed.\n");
	...
	if (mcan_moduleDeinit(init.idx) != EOK) {
		fprintf(stderr, "[MCAN DEMO] MCAN deinitialization failed.\n");
	}

	if (gpio_deinitModule(MCAN_GPIO_MODULE) != EOK) {
		fprintf(stderr, "[MCAN DEMO] GPIO deinitialization failed.\n");
	}
