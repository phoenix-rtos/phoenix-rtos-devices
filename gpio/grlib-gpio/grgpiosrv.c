...
static void handleDevCtl(grgpio_ctx_t *ctx, msg_t *msg)
{
...
static void msgHandler(void)
{
	...
	for (;;) {
		...
		switch (msg.type) {
			...
			case mtDevCtl:
				handleDevCtl(&grgpio_common.ctx, &msg);
				break;
...
int main(int argc, char *argv[])
{
	...
	char buf[8];
	if (snprintf(buf, sizeof(buf), "gpio%lu", portNum) >= (int)sizeof(buf)) {
		return EXIT_FAILURE;
	}

	if (create_dev(&grgpio_common.oid, buf) < 0) {
		portDestroy(grgpio_common.oid.port);
