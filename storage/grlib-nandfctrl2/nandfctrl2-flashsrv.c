...
static ssize_t flashsrv_devWriteRaw(id_t id, flash_i_devctl_t *idevctl, const char *data)
{
	...
	size_t rawEraseBlockSz = (strg->dev->mtd->erasesz / strg->dev->mtd->writesz) * (strg->dev->mtd->writesz + strg->dev->mtd->metaSize);
	size_t rawPartsz = (strg->size / strg->dev->mtd->erasesz) * rawEraseBlockSz;

	if ((rawoffs + rawsz) >= rawPartsz) {
		return -EINVAL;
	}

	size_t rawPagesz = strg->dev->mtd->writesz + strg->dev->mtd->metaSize;
	...
	while (tempsz < rawsz) {
		...
		res = flashdrv_writeraw(strg->dev->ctx->die, (uint32_t)(rawoffs / rawPagesz), 0U, strg->dev->ctx->databuf, rawPagesz);
		if (res < 0) {
...
static ssize_t flashsrv_devReadRaw(id_t id, flash_i_devctl_t *idevctl, char *data)
{
	...
	mutexLock(strg->dev->ctx->lock);
	while (tempsz < rawsz) {
		res = flashdrv_readraw(strg->dev->ctx->die, (uint32_t)(rawoffs / rawPagesz),
				0U, strg->dev->ctx->databuf, rawPagesz);
		if (res < 0) {
...
static int flashsrv_partAdd(blkcnt_t start, blkcnt_t size, const char *name, const char *rootPath)
{
	...
	oid_t poid;
	int err = flash_oidResolve(rootPath, &poid);
	if (err < 0) {
	...
	err = flashdev_init(strg, parent->dev->ctx->die->target);
	if (err < 0) {
		free(strg);
	...
	LOG("%-18s <%4llu, %4llu>: %s", path, start, start + size, (name != NULL) ? name : "(unnamed)");
