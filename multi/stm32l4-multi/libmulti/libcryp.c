...
/*
NOTE: The tinyaes implementation will have to be changed,
bcs CRYP excpects to initialize it's registers in a different order than AES on stm32nl4
Keep the API mostly the same but change how it's called in tinyaes
...
/* Stores veclen bytes from vector, lowest address to highest starting at reg register.
 * veclen has to be a multiple of 4
 */
...
/* Retrieves veclen bytes from lowest address to highest starting at reg register.
 * veclen has to be a multiple of 4
 */
...
int libcryp_setKey(const unsigned char *key, int keylen)
{
	...
	/* Check if key writing sequence succeded */
	if ((*(common.base + cryp_sr) & (0x1 << SR_KEYVALID)) == 0) {
		return -EINVAL;
	}
...
void libcryp_deriveDecryptionKey()
{
	unsigned int t;
	...
	*(common.base + cryp_cr) |= (0x1 << CR_FFLUSH);
	...
	t |= (0x1 << CR_ALGOMODE_H) | (0x3 << CR_ALGOMODE_L);
...
// Very work in progressssssssss
int tmp(unsigned char *key, unsigned char *iv, const unsigned char *in, unsigned char *out)
{
	unsigned int t;
	...
	*(common.base + cryp_cr) &= ~(0x3 << CR_KMOD);
	...
	libcryp_processBlock(in, out);
	// libcryp_setIv(out);

	libcryp_processBlock(in + 16, out + 16);
...
void libcryp_processBlock(const unsigned char *in, unsigned char *out)
{
	...
	// It should be enabled before the first block and disabled after the last one
	// libcryp_enable();
	...
	/* Treating incoming bytes as msb first and storing them as big endian */
	for (i = 0; i < 4; i++) {
		tmp = *in++ << 24;
		tmp |= *in++ << 16;
		tmp |= *in++ << 8;
