...
#define SR_TXP (1 << 1)  /* TX FIFO not full */
#define SR_RXP (1 << 0)  /* RX FIFO not empty */
...
static const struct libspi_peripheralInfo {
	...
	{ (uintptr_t)SPI1_BASE, pctl_spi1, pctl_ipclk_spi1sel, clkid_per },
	{ (uintptr_t)SPI2_BASE, pctl_spi2, pctl_ipclk_spi2sel, clkid_per },
	{ (uintptr_t)SPI3_BASE, pctl_spi3, pctl_ipclk_spi3sel, clkid_per },
	{ (uintptr_t)SPI4_BASE, pctl_spi4, pctl_ipclk_spi4sel, clkid_per },
	{ (uintptr_t)SPI5_BASE, pctl_spi5, pctl_ipclk_spi5sel, clkid_per },
	{ (uintptr_t)SPI6_BASE, pctl_spi6, pctl_ipclk_spi6sel, clkid_per },
};
...
int libspi_transaction(libspi_ctx_t *ctx, int dir, unsigned char cmd, unsigned int addr, unsigned char flags, unsigned char *ibuff, const unsigned char *obuff, size_t bufflen)
{
	...
	*(ctx->base + spi_cr1) |= 1;
	 ...
	 * They can be sent all at once because FIFO is large enough (16 bytes) */
	if ((flags & spi_cmd) != 0) {
		*(ctx->base + spi_txdr) = cmd;
		ignoreBytes += 1;
	}
	...
	while (ignoreBytes != 0) {
		while ((*(ctx->base + spi_sr) & SR_RXP) == 0) {
			/* Wait for a packet to arrive */
		}

		(void)*(ctx->base + spi_rxdr);
		ignoreBytes--;
	}

	for (size_t i = 0; i < bufflen; i++) {
		while ((*(ctx->base + spi_sr) & SR_TXP) == 0) {
			/* Wait for space in the FIFO */
		}

		if (dir != spi_dir_read) {
			*(ctx->base + spi_txdr) = obuff[i];
		}
		else {
			*(ctx->base + spi_txdr) = 0;
		}
		...
		if (dir != spi_dir_write) {
			ibuff[i] = *(ctx->base + spi_rxdr);
		}
		else {
			(void)*(ctx->base + spi_rxdr);
		}
...
int libspi_configure(libspi_ctx_t *ctx, char mode, char bdiv, int enable)
{
	...
	*(ctx->base + spi_cr1) &= ~1;
	...
	uint32_t bdiv_bits = ((((uint32_t)bdiv) & 0x3) << 28);
	...
	v |=
			...
			(0 << 16) | /* CRC not used */
			dma_bits |  /* DMA mode */
			(0 << 9) |  /* In case of underrun, send repeated pattern */
			(7 << 5) |  /* FIFO threshold - 8 frames (8 bytes) */
	...
	*(ctx->base + spi_cfg1) = v;
	...
	v &= ~0xf7fee0ff;
	v =
			(0 << 31) | /* SPI peripheral releases GPIOs when disabled */
	...
	*(ctx->base + spi_udrdr) = 0;     /* Underrun data pattern - all 0s */
