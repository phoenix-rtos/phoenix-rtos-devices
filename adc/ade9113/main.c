...
bool dmaRxCb(const uint8_t *data, size_t size)
{
	...
	for (int sample = 0; sample < size / 64; ++sample) {
		for (int chip = 0; chip < 4; ++chip) {
			...
			/* Invalid samples will contain MIN value for two's complement 24-bit number. This is outside of nominal ADE9113 range. */
			const uint8_t errorValue[3] = { 0x00, 0x00, 0x80 };
			bool valid = false;
			if (error != NULL) {
				ctx->crcErrors += 1;
			}
			...
			for (int ch = 0; ch < 3; ++ch) {
				if ((ctx->channels & (0x01 << (chip * 4 + ch))) != 0) {
					const uint8_t *chValue = valid ? &rsp[1 + 4 * ch] : errorValue;
					sample_write(&ctx->samples, chValue, 3);
				}
			}
		}
	}
...
{
	char *end = NULL;
	unsigned long int result = strtoul(value, &end, 10);
	if (end == NULL || *end != '\0') {
...
{
	printf("Usage: %s [OPTIONS] <DMA_RX_CH> <DMA_TX_CH>\n", (progname != NULL) ? progname : "prog");
	...
	puts("\t-e             Ignore (less critical) errors and failed checks");
	puts("\t-i             bitmask for selecting active channels. 4 bits per chip b0:I b1:V1 b2:V2 b3:ignored");
	puts("\t<DMA_RX_CH>,<DMA_TX_CH> SDMA channel paths ie. \"/dev/sdma/ch07\"");
}
...
static int parseArgs(struct args *parsed, int argc, char **argv)
{
	*parsed = (struct args) {
	...
	while (true) {
		int c = getopt(argc, argv, "hevm:s:b:c:i:");
		if (c == -1) {
			int posCount = argc - optind;
			if (posCount != 2) {
				log_error("invalid number of positional arguments expected=2 found=%d", posCount);
				return -1;
			}
			parsed->dmaRxCh = argv[optind];
			parsed->dmaTxCh = argv[optind + 1];
			break;
		}
		switch (c) {
			case 'm': {
			...
			case 's': {
				...
				if (value < 0) {
					log_error("invalid datapath (option `-s`)");
					return -1;
				}
			...
			case 'b': {
				...
				if (value < 0) {
					log_error("invalid sample buffer size (option `-b`)");
					return -1;
				}
			...
			case 'i': {
				char *end = NULL;
				unsigned long int result = strtoul(optarg, &end, 0);
				if (end == NULL || *end != '\0') {
					log_error("failed to parse input channels (option -i)");
					return -1;
				}
				if ((result < 0) || (result > UINT16_MAX)) {
					log_error("input channels value out of allowed range <%u;%u>", 1, UINT16_MAX);
					return -1;
				}
				parsed->enabledChannels = result;
				break;
			}
			case 'h':
			default: {
...
int gpioWrite(const char *path, uint32_t value, uint32_t mask)
{
	...
	const uint32_t data[2] = { value, mask }; /* using native endianness - same as in gpio driver */
...
int adcInit(struct adcCtx *ctx, addr_t spiBase, unsigned bufferSize, uint16_t enabledChannels)
{
	...
	ctx->channels = enabledChannels & 0x7777;
	unsigned enabledCount = 0;
	for (int i = 0; i < 16; ++i) {
		if ((ctx->channels & (1 << i)) != 0) {
			enabledCount += 1;
		}
	}
	return sample_init(&ctx->samples, bufferSize, enabledCount * 3);
}


static int waitReady(struct ade9113_ctx *ctx, bool all, time_t timeout)
{
	time_t now;
	gettime(&now, NULL);
	const time_t end = now + timeout;
	while (true) {
		bool ready = true;
		uint8_t values[4];
		ade9113_readRegs(ctx, ADE9113_STATUS0, values, 4);
		for (int i = 0; i < (all ? 4 : 1); ++i) {
			if (ade9113_status0_decode(values[i]).reset_done == 0) {
				ready = false;
				break;
			}
		}
		...
		if ((timeout > 0) && (now > end)) {
			log_error("ADC waitReady timeout");
			return -1;
		}
		usleep(2000);
	}
...
{
	log_info("Initializing ADC using SPI1");
	resetEcspi(ctx->spi);
	...
	if ((waitReady(&ade, true, 1000 * 1000) < 0) && !args->ignoreErrors) {
		return -1;
	}
	...
	/*
	 * Synchronize sampling between chips. From datasheet:
	 *
	 *   1. Write to SYNC_SNAP of all [...] devices to prepare for an ALIGN. Set PREP_BROADCAST = 0, ALIGN = 1, and SNAPSHOT = 0
	 *   2. Toggle the CS signal of all [...] devices simultaneously, which can be either a null SPI command with no SCLK toggles or part of a valid command.
	 *   3. Wait 3 cycles of DREADY for the update to take effect.
	 *
	 * Note that steps 1 and 2 are fulfilled in one "ade9113_writeRegs" as it issues "null" SPI command to read response
	 */
	ade9113_writeRegs(&ade, ADE9113_SYNC_SNAP, ADE9113_SYNC_SNAP_ENC(.align = 1));
	usleep(1 * 1000); /* ~ 8 cycles of DREADY (at least 3 are required) */

	/*
	 * Verify synchronization. From datasheet:
	 *
	 *   1. Write to SYNC_SNAP of all [...] devices to prepare for an ALIGN. Set PREP_BROADCAST = 0, ALIGN = 0, and SNAPSHOT = 1
	 *   2. Toggle the CS signal of all [...] devices simultaneously, which can be either a null SPI command with no SCLK toggles or part of a valid command.
	 *   3. Read back the ADC synchronization counter via the SNAPSHOT_COUNT_HI and SNAPSHOT_COUNT_LO registers.
	 *   4. Compare values from each [...] counter and ensure these values are within ±1 count difference between them.
	 */
	ade9113_writeRegs(&ade, ADE9113_SYNC_SNAP, ADE9113_SYNC_SNAP_ENC(.snapshot = 1));
	{
		uint16_t count[4];
		ade9113_readRegsU16(&ade, ADE9113_SNAPSHOT_COUNT_HI, count, 4);
		...
		/* counter values are still close to initial value so we do not need to be worried about wrap around (overflow) */
		if (max - min > 1) {
			log_info("counter snapshot: [0x%04x,0x%04x,0x%04x,0x%04x]", count[0], count[1], count[2], count[3]);
			log_error("ERROR: synchronization verification failed diff=%d (expected diff <= 1)", max - min);
			if (!args->ignoreErrors) {
				return -1;
			}
		}
	...
	for (int try = 0; try < 10; ++try) {
		...
		bool ready = true;
		for (int i = 0; i < 4; ++i) {
			if (ade9113_config_crc_decode(values[i]).crc_done == 0) {
				ready = false;
				break;
			}
		}
	...
	uint16_t crc[4];
	ade9113_readRegsU16(&ade, ADE9113_CRC_RESULT_HI, crc, 4);
...
{
	/* workaround for libphoenix `fprinf(stderr, ...)` splitting lines longer than 16 bytes */
	static char stderrBuff[512];
	setvbuf(stderr, stderrBuff, _IOLBF, sizeof(stderrBuff));
	...
	if (adcInit(ctx, IMX_ECSPI1_BASE, args.bufferSize, args.enabledChannels) < 0) {
		return EX_OSERR;
	};

	if (adcConfigure(ctx, &args) < 0) {
		log_error("failed to configure ADC's");
		return EX_IOERR;
	}

	if (dma_init(_PAGE_SIZE * 2, 2, args.dmaRxCh, args.dmaTxCh, dmaRxCb) < 0) {
	...
	static uint8_t msgStacks[2][_PAGE_SIZE] __attribute__((aligned));
