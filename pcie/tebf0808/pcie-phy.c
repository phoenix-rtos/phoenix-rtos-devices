...
static int pcie_phyDeassertGtReset(void)
{
	...
	if (ret != 0) {
		printf("pcie-phy: fail to deassert GT reset\n");
		return ret;
	}
...
static int pcie_phyAssertResets(void)
{
	...
	ret = platformctl(&ctl1);
	if (ret != 0) {
		printf("pcie-phy: fail to assert PCIE CTRL clock\n");
		return ret;
	}
	...
	if (ret != 0) {
		printf("pcie-phy: fail to assert PCIE BRIDGE clock\n");
		return ret;
	}
	...
	if (ret != 0) {
		printf("pcie-phy: fail to assert PCIE CFG clock\n");
		return ret;
	}
...
static int serdes_fixcal_code(uint32_t *base)
{
	...
	do {
		...
		if (mask_status == 0) {
			...
			printf("pcie-phy: fail, SERDES initialization timed out\n");
			return mask_status;
		}
...
unsigned long psu_serdes_init_data(uint32_t *base)
{
	...
	*/
	...
	if (NULL == base) {
		printf("pcie-phy: fail to map SIOU registers memory\n");
	}
...
static int pcie_phyDeassertPcieResets(void)
{
	...
	ret = platformctl(&ctl3);
	if (ret != 0) {
		printf("pcie-phy: fail to deassert PCIE BRIDGE reset\n");
		return ret;
	}
	...
	if (ret != 0) {
		printf("pcie-phy: fail to deassert PCIE CFG reset\n");
		return ret;
	}
...
int main(int argc, char **argv)
{
	...
	/* Wait for PCIe reference clock */
	oid_t oid;
	while (lookup("/dev/pcie-clock", NULL, &oid) < 0) {
		usleep(10000);
	}
	...
	if (NULL == serdes) {
		printf("pcie-phy: fail to map SERDES registers memory\n");
		return -1;
	}
	uint32_t *pcireg = mmap(NULL, PCIREG_SIZE,
			...
			PCIREG_ADDRESS);
	if (NULL == pcireg) {
		printf("pcie-phy: fail to map PCIE registers memory\n");
		return -1;
	}
	...
	if (ret != 0) {
		printf("pcie-phy: fail to config clock\n");
		return ret;
	}
	ret = pcie_phyDeassertGtReset();
	if (ret != 0) {
		printf("pcie-phy: fail to deassert reset on PS GTR\n");
		return ret;
	}
	ret = pcie_phyAssertResets();
	if (ret != 0) {
		printf("pcie-phy: fail to assert reset on all 3 PCIE peripherals\n");
		return ret;
	}
	int retNegated = serdes_fixcal_code(serdes);
	if (retNegated != 1) {
		ret = -1;
		printf("pcie-phy: fail to fix calibration of SERDES peripheral\n");
		return ret;
	}
	...
	if (ret != 0) {
		printf("pcie-phy: fail to deassert PCIE CFG and PCIE BRIDGE reset\n");
		return ret;
	}
	...
	if (ret != 0) {
		printf("pcie-phy: fail to deassert PCIE CTRL reset\n");
		return ret;
	}
	...
	/* Create the device file to sync next stage of PCIe driver */
	char *path = "/dev/pcie-phy";
	if (create_dev(&oid, path) < 0) {
		fprintf(stderr, "pcie-phy: cannot create device file\n");
		return EXIT_FAILURE;
	}
