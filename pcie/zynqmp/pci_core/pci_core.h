...
extern logger_ctx_t _pcie_logger_ctx;
...
/* TODO: Organize in RB tree for faster lookup by MSI payload */
/* TODO: Organize better by physical topology - required for TLP limits and power management */
...
typedef struct pci_context {
	...
	/* Organize devices in RB tree by vid and pid */
	...
	volatile uint32_t rc_status_reg;
	pci_irq_info_t irq_queue[PCI_CONTEXT_IRQ_BUF_SIZE];
	volatile int irq_queue_rd_ptr;
	volatile int irq_queue_wr_ptr;

	volatile bool irq_fifo_overflow;
	volatile bool should_unmask_irq;

	/* TODO: Organize devices for fast lookup by MSI payload*/
...
/* Get device from PCIe tree by vid and pid*/
