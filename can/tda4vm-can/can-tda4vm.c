...
static int mcan_irqHandler(unsigned int intr, void *data)
{
	...
	*(mcan->baseCore + ir) = irqStatus;
	atomic_fetch_or(&mcan->irqStatus, irqStatus);
...
static void mcan_irqThread(void *arg)
{
	...
	for (;;) {
		...
		for (;;) {
			pendingIRQ = atomic_exchange(&mcan->irqStatus, 0);
			if (pendingIRQ == 0UL) {
				break;
			}

			if (((pendingIRQ >> MCAN_IR_TC) & 1U) == 1U) {
				condSignal(mcan->txCond);
			}

			if (((pendingIRQ >> MCAN_IR_RF0N) & 1U) == 1U) {
				mutexLock(mcan->moduleLock);
			...
			if (((pendingIRQ >> MCAN_IR_RF1N) & 1U) == 1U) {
				mutexLock(mcan->moduleLock);
				if (mcan->rx.rxBusy == 1U) {
					if (mcan->rx.xferCount == mcan->rx.xferSize) {
						condSignal(mcan->rxCond);
...
static void mcan_parseTxMsg(struct mcan_txMessage *txMsg, struct mcu_mcan_module *mcan)
{
	...
	txMsg->xtd = mcan->extendedID;
	txMsg->esi = 0U;
	txMsg->data = mcan->tx.data->payload;
}


static int mcan_checkArgs(uint8_t idx)
{
	struct mcu_mcan_module *mcan;

	if ((idx != MCU_MCAN_MODULE0) && (idx != MCU_MCAN_MODULE1)) {
		fprintf(stderr, "[MCAN CORE] Invalid module index\n");
		return -EINVAL;
	}
	mcan = &gMcan[idx];
	if (mcan->init == 0U) {
		fprintf(stderr, "[MCAN CORE] Module not initialized\n");
		return -EIO;
	}
...
void mcan_getTxEvt(struct mcan_txEvent *txEvt)
{
	struct mcu_mcan_module *mcan;
	int ret;

	ret = mcan_checkArgs(txEvt->idx);
	if (ret != EOK) {
		return;
	}
	mcan = &gMcan[txEvt->idx];

	ret = mcan_readRAMTxEvt(mcan, txEvt);
	if (ret != EOK) {
		fprintf(stderr, "[MCAN CORE] Tx Event FIFO overflow\n");
		return;
	}
}
...
int mcan_txData(struct mcan_txReq *txReq)
{
	...
	/* sanitize args */
	if ((txReq->data == NULL) || (txReq->cnt == 0U)) {
		return -EINVAL;
	}
	ret = mcan_checkArgs(txReq->moduleIdx);
	if (ret != EOK) {
		return ret;
	}
	mcan = &gMcan[txReq->moduleIdx];
	errCnt = (uint8_t)(*(mcan->baseCore + ecr) & 0xFFU);
...
int mcan_rxData(struct mcan_rxReq *rxReq)
{
	...
	/* sanitize args */
	if ((rxReq->data == NULL) || (rxReq->cnt == 0U)) {
		return -EINVAL;
	}
	ret = mcan_checkArgs(rxReq->moduleIdx);
	if (ret != EOK) {
		return ret;
	}
	mcan = &gMcan[rxReq->moduleIdx];
	errCnt = (uint8_t)((*(mcan->baseCore + ecr) >> MCAN_ECR_REC) & 0x7FU);
	...
	if ((uint8_t)((*(mcan->baseCore + ecr) >> MCAN_ECR_REC) & 0x7FU) != errCnt) {
		return -EIO;
	}
...
int mcan_moduleDeinit(uint8_t idx)
{
	int ret;
	ret = mcan_checkArgs(idx);
	if (ret != EOK) {
		return ret;
	}
