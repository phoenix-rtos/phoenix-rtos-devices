...
int pwm_setBitSequence(pwm_tim_id_t timer, pwm_ch_id_t chn, void *data, uint32_t nbits, uint8_t datasize, int flags)
{
	...
	for (unsigned int i = 0; i < nbits; i++) {
		// uint32_t compare = (*(uint32_t *)(((uint8_t *)data) + (i * datasize))) & (0xffffffff >> ((4 - datasize) * 8));
...
#if 0
		/* Set address that DMA transfers data to. This seems broken */
		*(pwm_setup.timer[timer].base + tim_dcr) =
				(1 << 16) |                       /* DMA burst source - UEV */
				(0 << 8) |                        /* DMA burst length - 1 */
				((PWM_CCR_REG(chn) & 0x1f) << 0); /* DMA base address - CCRx */
#endif
