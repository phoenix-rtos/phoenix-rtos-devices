...
/* clang-format off */
/* FIXME: disabled for now as printf and other libphoenix functions that write to stdout might deadlock as we're using only one lock. */
...
{
	return libtty_read_nonblock(tty, data, size, mode, NULL);
}
