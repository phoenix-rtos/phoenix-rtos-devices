...
#define MCSPI_MODULE_OFF   0U
#define MCSPI_MODULE_INIT  1U
#define MCSPI_MODULE_READY 2U
...
static void _mcspi_txInit(struct mcspi_dev *dev)
{
	...
	if (dev->type == MCSPI_DEVICE_SLAVE) {
		...
		if (wordCnt > xfer->xferCntTx) {
			wordCnt = xfer->xferCntTx;
		}
...
int mcspi_xferStart(struct mcspi_xferReq *xferReq)
{
	...
	if (xferReq->devnum > MCU_MCSPI2) {
		return -EINVAL;
	}
	dev = &mcspiDev[xferReq->devnum];
	...
	/* check if module was not deinitialized while waiting for lock */
	if (dev->initialized != MCSPI_MODULE_READY) {
		mutexUnlock(dev->devMutex);
		return -EIO;
	}
	...
	*(dev->base + chconf + dev->choffs) |= fifoState;
	...
	/* if trigger level == FIFO size, decrement trigger level so no underrun occurs */
	if (xfer->rxTrig == xfer->fifoSize) {
		xfer->rxTrig -= 4U;
	}
	if (xfer->txTrig == xfer->fifoSize) {
		xfer->txTrig -= 4U;
	}
...
int mcspi_deinit(uint8_t devnum)
{
	...
	dev->initialized = MCSPI_MODULE_OFF;
	mutexUnlock(dev->devMutex);
...
int mcspi_init(uint8_t devnum, uint8_t chnum, uint8_t devtype, struct mcspi_modulctrl *modctrl, struct mcspi_chconf *chconf)
{
	...
	if (dev->initialized != MCSPI_MODULE_OFF) {
		return -EBUSY;
	}
	dev->initialized = MCSPI_MODULE_INIT;
	...
	*(dev->base + irqstatus) = UINT32_MAX;
	...
	dev->initialized = MCSPI_MODULE_READY;
