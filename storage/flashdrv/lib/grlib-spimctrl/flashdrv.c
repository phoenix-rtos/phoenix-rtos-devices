...
static int _flashdrv_mtdRead(storage_t *strg, off_t offs, void *buff, size_t len, size_t *retlen)
{
	...
	flash_readData(ctx->dev, ctx->spimctrl, offs, buff, len);
...
static int _flashdrv_mtdWrite(storage_t *strg, off_t offs, const void *buff, size_t len, size_t *retlen)
{
	...
	while (doneBytes < len) {
		...
		res = flash_pageProgram(ctx->dev, ctx->spimctrl, offs, src, chunk,
...
static int flashdrv_mtdErase(storage_t *strg, off_t offs, size_t len)
{
	...
	if ((offs == 0) && (len == CFI_SIZE(ctx->cfi.chipSz))) {
		...
		res = flash_chipErase(ctx->dev, ctx->spimctrl, CFI_TIMEOUT_MAX_ERASE(ctx->cfi.toutTypical.chipErase, ctx->cfi.toutMax.chipErase));
	...
	if (res == -ENOSYS) {
		off_t secOffs = offs;
		while (secOffs < end) {
			res = flash_sectorErase(ctx->dev, ctx->spimctrl, secOffs, CFI_TIMEOUT_MAX_ERASE(ctx->cfi.toutTypical.blkErase, ctx->cfi.toutMax.blkErase));
			if (res < 0) {
				break;
			}
