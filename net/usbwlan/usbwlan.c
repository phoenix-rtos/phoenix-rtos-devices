...
static struct {
	...
	int msgtid;
	handle_t lock;
	char stack[USBWLAN_MSGTHREAD_STACK_SIZE] __attribute__((aligned(8)));
} usbwlan_common;
...
static int usbwlan_handleInsertion(usb_driver_t *drv, usb_devinfo_t *ins, usb_event_insertion_t *event_out)
{
	...
	do {
		...
		err = usb_setConfiguration(dev->drv, dev->pipeCtrl, 1);
		if (err < 0) {
			usbwlan_log("Failed to set configuration");
			err = -EINVAL;
			break;
		}
...
static int usbwlan_handleCompletion(struct usb_driver *drv, usb_completion_t *completion, const char *data, size_t len)
{
	...
	do {
		...
		else if (dev->rx.msg.o.size < len) {
			/* provided buffer was too small */
			dev->rx.msg.o.err = -EINVAL;
		}
		else {
	...
	for (;;) {
		...
		switch (msg.type) {
			...
			case mtRead:
				...
				mutexUnlock(dev->rx.lock);
				if (err == -EWOULDBLOCK) {
					/* response in usbwlan_handleCompletion */
					usbwlan_put(dev);
					continue;
				}
				else {
...
static int usbwlan_destroy(usb_driver_t *drv)
{
	...
	if (usbwlan_common.msgport != 0) {
		...
		usbwlan_common.msgport = 0;
		if (usbwlan_common.msgtid != 0) {
			threadJoin(usbwlan_common.msgtid, 0);
		}
	}
	...
	memset(&usbwlan_common, 0, sizeof(usbwlan_common));
...
static int usbwlan_init(usb_driver_t *drv, void *arg)
{
	...
	do {
		...
		ret = beginthreadex(usbwlan_msgthread, USBWLAN_MSGTHREAD_PRIO, usbwlan_common.stack, sizeof(usbwlan_common.stack), NULL, &usbwlan_common.msgtid);
		if (ret < 0) {
			usbwlan_log("Failed to start msgthread");
			break;
		}
	} while (0);
