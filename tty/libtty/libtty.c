	...
	*out_arg = NULL;
	...
	switch (cmd) {
		...
		case TIOCSHALFD:
			...
			log_ioctl("TIOCSHALFD: enable=%d", (unsigned int)in_arg);
			if ((unsigned int)in_arg > 1U) {
				log_warn("halfduplex enable (%u) > 1", (unsigned int)in_arg);
				return -EINVAL;
			}
			CALLBACK(set_halfduplex, (unsigned int)in_arg);
			break;
		default:
