...
static struct {
	...
	{ .active = SPWRTR0_ACTIVE },
	{ .active = SPWRTR1_ACTIVE },
	{ .active = SPWRTR2_ACTIVE },
	{ .active = SPWRTR3_ACTIVE },
	{ .active = SPWRTR4_ACTIVE },
	{ .active = SPWRTR5_ACTIVE },
};


static struct {
	oid_t oid;
	spwrtr_dev_t dev[SPWRTR_CNT];
} spwrtr_common;
...
static int spwrtr_resetDevice(spwrtr_dev_t *dev)
{
	...
	(void)mutexUnlock(dev->resetLock);
...
static void spwrtr_handleDevCtl(msg_t *msg, int dev)
{
	...
	switch (ictl->type) {
		case spwrtr_pmap_set:
			err = spwrtr_setPortMapping(&spwrtr_common.dev[dev], ictl->task.mapping.port, ictl->task.mapping.enPorts);
			break;

		case spwrtr_pmap_get:
			err = spwrtr_getPortMapping(&spwrtr_common.dev[dev], ictl->task.mapping.port, &octl->val);
			break;

		case spwrtr_reset:
			err = spwrtr_resetDevice(&spwrtr_common.dev[dev]);
			break;
...
static void spwrtr_dispatchMsg(msg_t *msg)
{
	id_t dev = msg->oid.id;
...
static int spwrtr_createDevs(oid_t *oid)
{
	for (unsigned int i = 0; i < SPWRTR_CNT; i++) {
		if (spwrtr_info[i].active == 0) {
			continue;
		}

		char buf[11];
		if (snprintf(buf, sizeof(buf), "spwrtr%d", i) >= sizeof(buf)) {
			return -1;
		}
...
static int spwrtr_createResources(spwrtr_dev_t *dev, addr_t pbase)
{
	dev->mmio = MAP_FAILED;
	dev->resetLock = (handle_t)-1;
	dev->pmapLock = (handle_t)-1;
	...
	if (mutexCreate(&dev->resetLock) < 0) {
		return -1;
	}

	if (mutexCreate(&dev->pmapLock) < 0) {
		return -1;
	}
...
static void spwrtr_cleanupResources(spwrtr_dev_t *dev)
{
	if (dev->resetLock != (handle_t)-1) {
		resourceDestroy(dev->resetLock);
	}

	if (dev->pmapLock != (handle_t)-1) {
		resourceDestroy(dev->pmapLock);
	}
	...
	for (unsigned int i = 0; i < SPWRTR_CNT; i++) {
		...
		for (int j = 0; j < 4; j++) {
			if (dev.info.ahb.type[j] == AMBA_TYPE_AHBIO) {
				spwrtr_info[i].base = dev.info.ahb.base[j];
				break;
			}
		}
		spwrtr_info[i].irq = dev.irqn;

		if (spwrtr_createResources(&spwrtr_common.dev[i], (addr_t)spwrtr_info[i].base) < 0) {
			LOG_ERROR("Failed to create resources");
			spwrtr_cleanupResources(&spwrtr_common.dev[i]);
			return -1;
		}
	}
	return 0;
...
int main(void)
{
	...
	if (spwrtr_createDevs(&spwrtr_common.oid) < 0) {
		portDestroy(spwrtr_common.oid.port);
		LOG_ERROR("Failed to create SpaceWire-router devices");
		return EXIT_FAILURE;
	}
