...
#define PCIE_LOG_FILE_PATH "pcie_root.log"
logger_ctx_t _pcie_logger_ctx = { 0 };

static int logger_file_callback(logger_ctx_t ctx, const void *buffer, size_t len, void *arg)
{
	int fd = *(int *)arg;
	ssize_t ret;
	size_t wrote = 0;
	...
	while (wrote != len) {
		ret = write(fd, &((char *)buffer)[wrote], len - wrote);
		if (ret < 0) {
			if (ret != -EINTR)
				return -EAGAIN;
		}
...
static int pci_initLogger(void)
{
	struct logger_options_t opts = { .delay_milis = 100, .queue_size = _PAGE_SIZE };
	_pcie_logger_ctx = logger_init(&opts);

	int *fd = malloc(sizeof(*fd));
	*fd = open(PCIE_LOG_FILE_PATH, O_CREAT | O_RDWR, 0777);
	logger_push_callback(_pcie_logger_ctx, logger_file_callback, fd);
...
void pci_enumerateRoot(uint32_t *pcie, pci_dev_t *root, uint8_t bus, int depth)
{
	...
	for (uint8_t dev = 0; dev < (bus ? 32 : 1); ++dev) {
		...
		if (NULL == base_function) {
			logger_log(_pcie_logger_ctx, "pcie: Failed to allocate memory for PCI dev during enumeration, level :%d\n", depth);
			return;
		}
		...
		if (multi) {
			logger_log(_pcie_logger_ctx, "pcie: multiple func device %u\n", dev);
...
void pci_scanFunc(uint32_t *pcie, pci_dev_t *device, int depth)
{
	...
	logger_log(_pcie_logger_ctx, "pcie: %02x:%02x.%u ven %04x dev %04x class %02x%02x%02x hdr 0x%02x\n",
			bus, dev, fun,
			vid, pid,
	...
	if (!(cmd & (PCI_CMD_MEM | PCI_CMD_MASTER))) {
		logger_log(_pcie_logger_ctx, "pcie: enable memory space and bus master\n");
	...
	if (hdr == 0x01) {
		...
		logger_log(_pcie_logger_ctx, "DRIVER: CONFIGURE BRIDGE\n");
		pci_setMaxPayloadSize(pcie, device);
	}
	else {
		/* If this is a PCI endpoint, configure device and return */
		logger_log(_pcie_logger_ctx, "DRIVER: CONFIGURE ENDPOINT\n");
...
void pci_configureBridge(uint32_t *pcie, pci_dev_t *device, int depth)
{
	...
	if (sec == 0) {
		...
		logger_log(_pcie_logger_ctx, "pcie: write pri, sec, sub 0x%x \n", val);
		...
		logger_log(_pcie_logger_ctx, "pcie: validate pri, sec, sub 0x%x \n", val_read);
	}

	uint8_t sub = ecamRead8((uintptr_t)pcie, bus, dev, fun, PCI_SUBORDINATE_BUS);
	logger_log(_pcie_logger_ctx, "pcie: bridge bus primary %u secondary %u subordinate %u\n",
			bus, sec, sub);
	...
	logger_log(_pcie_logger_ctx, "pcie: Setting bridge bus to PCIe address translation\n");

	/* Configure translations */
	writeReg(pcie, 0x208, (uint32_t)(PCI_64P_BAR_BA >> 32));
	writeReg(pcie, 0x20C, (uint32_t)(PCI_64P_BAR_BA & ~0u));

	writeReg(pcie, 0x210, (uint32_t)(PCI_64P_BAR_BA >> 32));
	writeReg(pcie, 0x214, (uint32_t)(PCI_64P_BAR_BA & ~0u));

	writeReg(pcie, 0x218, (uint32_t)(PCI_64P_BAR_BA >> 32));
	writeReg(pcie, 0x21C, (uint32_t)(PCI_64P_BAR_BA & ~0u));

	writeReg(pcie, 0x220, (uint32_t)(PCI_64P_BAR_BA >> 32));
	writeReg(pcie, 0x224, (uint32_t)(PCI_64P_BAR_BA & ~0u));

	writeReg(pcie, 0x228, (uint32_t)(PCI_64P_BAR_BA >> 32));
	writeReg(pcie, 0x22C, (uint32_t)(PCI_64P_BAR_BA & ~0u));

	writeReg(pcie, 0x230, (uint32_t)(PCI_64P_BAR_BA >> 32));
	writeReg(pcie, 0x234, (uint32_t)(PCI_64P_BAR_BA & ~0u));

	/* Write address range for MSI */
	writeReg(pcie, 0x14C, (uint32_t)(PCI_MSI_BA >> 32));
	writeReg(pcie, 0x150, (uint32_t)(PCI_MSI_BA & ~0x0u));

	logger_log(_pcie_logger_ctx, "pcie: Managed to set bridge bus to PCIe address translation\n");
...
void pci_configureCapBridge(uint32_t *pcie, pci_dev_t *device)
{
	...
	while (ptr >= 0x40) {
		...
		if (0x10 == cap_id) {
			...
			logger_log(_pcie_logger_ctx, "pcie: %d:%d.%d Current device control 2 is %x\n", bus, dev, fun, reg);
			return;
		}
...
void pci_bridgeEnable(uint32_t *pcie, pci_dev_t *device)
{
	...
	logger_log(_pcie_logger_ctx, "pcie: Trying to enable bridge\n");
	...
	logger_log(_pcie_logger_ctx, "pcie: Root bridge\n");
}
	...
	for (int i = 0; i < bar_count; i++) {
		...
		if (io) {
			...
			if ((uintptr_t)-1 == bar_phys_add) {
				logger_log(_pcie_logger_ctx, "pcie: %d:%d.%d - Failed to allocate physical memory range for I/O BAR%d\n",
						bus, dev, fun, i);
				continue;
			}
			...
			if ((uintptr_t)-1 == bar_phys_add) {
				logger_log(_pcie_logger_ctx, "pcie: %d:%d.%d - Failed to allocate physical memory range for non-prefetchble 32-bit BAR%d\n",
						bus, dev, fun, i);
				continue;
			}
			...
			if ((uintptr_t)-1 == bar_phys_add) {
				logger_log(_pcie_logger_ctx, "pcie: %d:%d.%d - Failed to allocate physical memory range for prefetchble 32-bit BAR%d\n",
						bus, dev, fun, i);
				continue;
			}
			...
			if ((uintptr_t)-1 == bar_phys_add) {
				logger_log(_pcie_logger_ctx, "pcie: %d:%d.%d - Failed to allocate physical memory range for 64-bit BAR%d\n",
						bus, dev, fun, i);
				i++;
				continue;
			}
		...
		if ((uintptr_t)MAP_FAILED == device->bar[j]) {
			logger_log(_pcie_logger_ctx, "pcie: %d:%d.%d - Failed to map BAR%d: wanted %016lx:%lx\n",
...
void pci_setMaxPayloadSize(uint32_t *pcie, pci_dev_t *device)
{
	...
	logger_log(_pcie_logger_ctx, "DRIVER: PCIe device control register: 0x%x\n", reg);
	...
	logger_log(_pcie_logger_ctx, "DRIVER: PCIe device control register: 0x%x (should be 0x%x)\n", test, reg);
}
...
void pci_disableCompletionTimeout(uint32_t *pcie, pci_dev_t *device)
{
	...
	logger_log(_pcie_logger_ctx, "DRIVER: PCIe device control register 2: 0x%x\n", dev_ctrl2_reg);
	dev_ctrl2_reg |= (1 << 4);
	...
	logger_log(_pcie_logger_ctx, "DRIVER: PCIe device control register 2: 0x%x (should be 0x%x)\n", test, dev_ctrl2_reg);
}
...
void pci_enableTimeoutTLP(uint32_t *pcie, pci_dev_t *device)
{
	...
	while (ptr >= 0x40) {
		...
		if (cap_id == 0x01) {
			logger_log(_pcie_logger_ctx, "pcie: Current value of AER UEMR is 0x%x\n", ecamRead32((uintptr_t)pcie, bus, dev, fun, (ptr + 0x08)));
			ecamWrite32((uintptr_t)pcie, bus, dev, fun, ptr + 0x08, ~(1u << 14));
			logger_log(_pcie_logger_ctx, "pcie: Updated value of AER UEMR is 0x%x\n", ecamRead32((uintptr_t)pcie, bus, dev, fun, (ptr + 0x08)));
		}
...
int pci_initContext(pci_context_t *context)
{
	...
	context->irq_fifo_overflow = false;
	context->should_unmask_irq = false;
	// context->irq_queue_wr_ptr++;
	...
	if (phy_link_status.link_up) {
		...
		logger_log(_pcie_logger_ctx, "pcie: PHY LINK DOWN, phy status reg: 0x%x\n", phy_link_status.raw_register_val);
		return -EAGAIN;
	}
	...
	logger_log(_pcie_logger_ctx, "pcie: Managed to enumerate root complex\n");
	...
	logger_log(_pcie_logger_ctx, "pcie: Allocated resources for irq handling thread\n");

	context->stop_thread = false;
	context->lock = irq_handler_thread_args->mutex;
	context->cond = irq_handler_thread_args->cond;
	context->irq_thread_id = beginthread(pci_irqHandlerThread,
			PCI_IRQ_HANDLER_THREAD_PRIO, irq_handler_stack,
			PCI_IRQ_HANDELR_THREAD_STACK_SIZE, irq_handler_thread_args);

	logger_log(_pcie_logger_ctx, "pcie: Started irq handling thread\n");

	interrupt(PCI_ROOT_COMPLEX_IRQ_NO, pci_irqCallback, context,
			irq_handler_thread_args->cond, NULL);

	logger_log(_pcie_logger_ctx, "pcie: Registered irq handler\n");
	...
	logger_log(_pcie_logger_ctx, "pcie: Root complex enabled\n");

	return 0;
}

int pci_closeContext(pci_context_t *context)
{
	mutexLock(context->lock);
	context->stop_thread = true;
	mutexUnlock(context->lock);
	condSignal(context->cond);

	threadJoin(context->irq_thread_id, 100);
	/* TODO: perform cleanup safely turn off devices and close PCIe RC */
...
int pci_irqCallback(unsigned int irq_no, void *arg)
{
	...
	while ((readReg(context->pcie, 0x148) & (1u << 18)) != 0) {
		...
		/* Acquire filed only after it was filled */
...
void pci_irqHandlerThread(void *data)
{
	...
	while (1) {
		...
		if (context->stop_thread) {
			logger_log(_pcie_logger_ctx, "pci: Exit interrupt thread\n");
			mutexUnlock(arg->mutex);
			break;
		}

		logger_log(_pcie_logger_ctx, "pci: Received IRQ - %lu : %d/%d - RC status - 0x%x\n", counter++,
				context->irq_queue_rd_ptr, context->irq_queue_wr_ptr, context->rc_status_reg);

		if (context->irq_fifo_overflow) {
			logger_log(_pcie_logger_ctx, "pci: Interrupt FIFO overflow\n");
			exit(1);
		}

		if (0x1 == context->rc_status_reg) {
			/* Link down somehow? */
			phy_link_status phy_link_status = pci_checkLinkStatus(context->pcie);
			if (phy_link_status.link_up) {
				logger_log(_pcie_logger_ctx, "pcie: phy LINK UP, link rate: %u, link width x%u, ltssm state: %s\n",
		...
		for (;;) {
			int rd_ptr = atomic_load(&context->irq_queue_rd_ptr);
			int wr_ptr = atomic_load(&context->irq_queue_wr_ptr);
			pci_irq_info_t *irq_info = &(context->irq_queue[rd_ptr]);
			...
			/* Consume after interrupt was handled */
			atomic_store(&context->irq_queue_rd_ptr, (rd_ptr + 1) % PCI_CONTEXT_IRQ_BUF_SIZE);
		}

		if (context->should_unmask_irq) {
			/* Enable irq pin assertion */
			writeReg(context->pcie, 0x13c, ~0u);
			context->should_unmask_irq = false;
		}
...
void pci_resetAXIandRC(pci_context_t *context)
{
	...
	*/
	...
	logger_log(_pcie_logger_ctx, "pcie: Reseting the bus: %d\n", reg & (1 << 22) >> 22);
	...
	reg = ecamRead32((uintptr_t)context->pcie, 0, 0, 0, 0x3e);
	logger_log(_pcie_logger_ctx, "pcie: Current secondary bus reset value %d\n", reg & (1 << 22) >> 22);
...
void pci_readBridgeInfoReg(uint32_t *pcie)
{
	...
	logger_log(_pcie_logger_ctx, "pcie: bridge generation: %s, root port: %s\n",
...
void pci_printTree(pci_dev_t *root, int depth)
{
	...
	for (int i = 0; i < depth; i++) {
		logger_log(_pcie_logger_ctx, "%*s", depth, "\t");
	}
	logger_log(_pcie_logger_ctx, "-%d:%d.%d\n", root->bus_no, root->dev_no, root->func_no);
...
void pci_printBARs(uint32_t *pcie, pci_dev_t *device)
{
	...
	for (int i = 0; i < bar_count; i++) {
		...
		if (0x1 == (bar_low & 0x1)) {
			logger_log(_pcie_logger_ctx, "pcie: BAR%d I/O 0x%08x\n", i, bar_low & ~0x3u);
		}
		else {
			bool is_64_bit = (bar_low & 0x4) == 0x4;
			bool prefetchble = (bar_low & (1 << 3)) != 0;
			...
			logger_log(_pcie_logger_ctx, "pcie: BAR%d MEM 0x%016llx (%s), (%s)\n",
...
void print_capabilities(uint32_t *pcie, pci_dev_t *device)
{
	...
	while (ptr >= 0x40) {
		...
		logger_log(_pcie_logger_ctx, "pcie: CAP id 0x%02x address 0x%02x\n", cap_id, ptr);
...
{
	/*
	Since RC supports only one interrup line to APU and there
	is only single FIFO for interrupts, there is no need to provide
	many MSI ranges
	*/
	return PCI_MSI_BA;
}
