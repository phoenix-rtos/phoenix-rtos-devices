...
static int spwrtr_setClockDiv(spwrtr_dev_t *dev, uint8_t port, uint32_t clkdiv)
{
	if (port == 0 || port > SPWRTR_SPW_CNT || (clkdiv & ~0xFF) > 0) {
		return -EINVAL;
	}
...
static int spwrtr_getClockDiv(spwrtr_dev_t *dev, uint8_t port, uint32_t *clkdiv)
{
...
static void spwrtr_handleDevCtl(msg_t *msg)
{
	...
	switch (ictl->type) {
		...
		case spwrtr_clkdiv_get:
			err = spwrtr_getClockDiv(&spwrtr_common.dev, ictl->task.clkdiv.port, &octl->val);
			break;
