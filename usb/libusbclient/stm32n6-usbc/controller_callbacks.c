...
void clbc_epStall(stm32n6_endpt_t *ep)
{
	...
	/* stall OUT endpoint */
	if (ep->out.epNum != 0U) {
		USB_REG(otg_doepctl0 + ep->out.epNum * EP_STRIDE) |= (1UL << DOEPCTL_EPDIS);
	}

	USB_REG(otg_doepctl0 + ep->out.epNum * EP_STRIDE) |= (1UL << DOEPCTL_STALL);
}
...
static void epStartXfer(stm32n6_endpt_data_t *epData)
{
	...
	if (epData->is_in == 1U) {
		/* IN Endpoint */
		USB_REG(otg_dieptsiz0 + epData->epNum * EP_STRIDE) &= ~(0x7FFFFUL);
		USB_REG(otg_dieptsiz0 + epData->epNum * EP_STRIDE) &= ~(0x3FFUL << 19);

		if (epData->xfer_len == 0U) {
			/* Zero Length Packet */
			USB_REG(otg_dieptsiz0 + epData->epNum * EP_STRIDE) |= (1UL << 19);
		}
		else {
			...
			if (epData->epNum == 0U) {
				...
				USB_REG(otg_dieptsiz0) |= (1UL << 19);
				USB_REG(otg_dieptsiz0) |= (0x7FFFFUL & epData->xfer_len);
			}
			/* EP1-EP8 - whole transfer at once */
			else {
				pktcnt = (uint16_t)((epData->xfer_size + epData->maxpacket - 1U) / epData->maxpacket);
				USB_REG(otg_dieptsiz0 + epData->epNum * EP_STRIDE) |= ((pktcnt & 0x3FFUL) << 19);
				...
				USB_REG(otg_dieptsiz0 + epData->epNum * EP_STRIDE) |= (0x7FFFFUL & epData->xfer_size);
			}
		}

		/* enable endpoint and clear NAK for this transfer */
		USB_REG(otg_diepctl0 + epData->epNum * EP_STRIDE) |= ((1UL << 26) | (1UL << 31));
		...
		if (epData->type != USB_EP_TYPE_ISOC) {
			...
			if ((USB_REG(otg_dsts) & (1 << 8)) == 0U) {
				USB_REG(otg_diepctl0 + epData->epNum * EP_STRIDE) |= (1UL << 29);
			}
			else {
				USB_REG(otg_diepctl0 + epData->epNum * EP_STRIDE) |= (1UL << 28);
			}
			/**
		...
		/* OUT Endpoint */
		USB_REG(otg_doeptsiz0 + epData->epNum * EP_STRIDE) &= ~(0x3FFUL << 19UL);
		USB_REG(otg_doeptsiz0 + epData->epNum * EP_STRIDE) &= ~(0x7FFFFUL);
		...
		USB_REG(otg_doeptsiz0 + epData->epNum * EP_STRIDE) |= (0x7FFFFUL & epData->xfer_size);
		USB_REG(otg_doeptsiz0 + epData->epNum * EP_STRIDE) |= ((pktcnt & 0x3FFUL) << 19);

		if (epData->type == USB_EP_TYPE_ISOC) {
			if ((USB_REG(otg_dsts) & (1 << 8)) == 0U) {
				USB_REG(otg_doepctl0 + epData->epNum * EP_STRIDE) |= (1UL << 29);
			}
			else {
				USB_REG(otg_doepctl0 + epData->epNum * EP_STRIDE) |= (1UL << 28);
			}
		}
		USB_REG(otg_doepctl0 + epData->epNum * EP_STRIDE) |= ((1UL << 26) | (1UL << 31));
	}
}
