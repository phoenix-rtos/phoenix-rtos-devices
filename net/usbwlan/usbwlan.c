...
static struct {
	idtree_t devices;
	uint32_t msgport;
	int msgtids[USBWLAN_N_MSGTHREADS];
	handle_t lock;
...
static int usbwlan_destroy(usb_driver_t *drv)
{
	...
	if (usbwlan_common.msgport != 0) {
		...
		for (size_t i = 0; i < USBWLAN_N_MSGTHREADS; i++) {
			if (usbwlan_common.msgtids[i] != 0) {
				threadJoin(usbwlan_common.msgtids[i], 0);
			}
		}
	}
...
static int usbwlan_init(usb_driver_t *drv, void *arg)
{
	...
	do {
		...
		for (size_t i = 0; i < USBWLAN_N_MSGTHREADS; i++) {
			ret = beginthreadex(usbwlan_msgthread, USBWLAN_MSGTHREAD_PRIO, usbwlan_common.stacks[i], sizeof(usbwlan_common.stacks[i]), NULL, &usbwlan_common.msgtids[i]);
			if (ret < 0) {
