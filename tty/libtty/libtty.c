 ...
 *
 * Only the owning session is let through. With claim != 0 an unclaimed terminal
 * is accepted as well, so that the sender may take it over with
 * libtty_sessionClaim().
 */
static int libtty_sessionCheck(libtty_common_t *tty, pid_t sender_pid, int claim)
{
	 ...
	 *
	 * FIXME: a session that outlived its leader is assumed gone, although
	 * POSIX keeps it alive - the terminal is then released (and may be claimed
	 * by another session) too early.
	 */
...
/* Claims the terminal if unclaimed. Must be preceded by a successful libtty_sessionCheck(tty, sender_pid, 1). */
...
static int libtty_cttyIoctl(libtty_common_t *tty, pid_t sender_pid, unsigned int cmd, const void *in_arg, const void **out_arg)
{
	...
	switch (cmd) {
		...
		case TIOCSCTTY:
			...
			sid = ret;
			/* POSIX: only a session leader may acquire a controlling terminal */
			if (sid != sender_pid) {
				ret = -EPERM;
				break;
			}
