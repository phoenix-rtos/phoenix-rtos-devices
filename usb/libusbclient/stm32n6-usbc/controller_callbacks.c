...
void clbc_epTransmit(uint8_t epNum, uint8_t *dataBuf, uint32_t len)
{
	...
	if (epNum == 2U) {
		...
		/**
		 * from STM32F405/407xx errata:
		 * When the USB on-the-go full-speed peripheral is in Device mode, interrupting transmit FIFO write sequence with
		 * read or write accesses to OTG_FS endpoint-specific registers (those ending in 0 or x) leads to corruption of the
		 * next data written to the transmit FIFO.
		 *
		 * Apparently OTG in STM32N6 has the same problem, so disabling SOF interrupts for smooth transmission.
		 * If problem persists, disabling other interrupts or servicing TXFE interrupt in ISR should solve it.
		 */

		USB_REG(GINTMSK) &= ~(1UL << GINTSTS_SOF);
	}
...
void _clbc_IEPINT(void)
{
	...
	while (daintClear != 0U) {
		...
		if ((daintClear & 0x1U) != 0) {
			...
			if (IS_IRQ(epInt, DIEPINT_TXFE)) {
				clbc_common.dc->irqPendingDIEPINT[epNum] &= ~(1UL << DIEPINT_TXFE);
