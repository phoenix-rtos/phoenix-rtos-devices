	...
	while (true) {
		...
		switch (msg.type) {
			case mtOpen: {
				...
				for (unsigned ch = 0; ch < MSGAPI_READER_COUNT; ++ch) {
					if (!ctx->readers[ch].open) {
						...
						log_info("new msgapi session: ch=%d pid=%d opened=%u", ch + 1, msg.pid, ctx->opened);
						break;
					}
				}
				...
				if (msg.o.err == -EBUSY) {
					log_error("failed to allocate msgapi session: pid=%d opened=%u", msg.pid, ctx->opened);
				}
				break;
			}
			...
			case mtClose: {
				...
				msg.o.err = EOK;
				ctx->opened -= 1;
				log_info("closing msgapi session: ch=%d pid=%d opened=%u", (int)msg.oid.id, msg.pid, ctx->opened);
				mutexUnlock(ctx->lock);
				break;
			}
			...
			case mtGetAttr: {
				...
				switch (msg.i.attr.type) {
					case atPollStatus:
						msg.o.attr.val = POLLRDNORM;
						if (sample_readPoll(reader) > 0) {
