...
static void resetDescriptor(struct sdmaChannel *ch, size_t idx)
{
	...
	/* TODO: check this line can be skipped when handling IRQ */
	bd[idx] = (struct sdma_buffer_desc_s) {
		...
		/* `ch->buffer.count == 1` is a special case for tx that reuses single buffer */
...
static int sdmaInit(struct sdmaCtx *const ctx, size_t size, size_t count, const char *rxChannel, const char *txChannel)
{
	...
	/*
	 * For continuous operation of data collection we need to ensure that one full buffer is read before second
	 * buffer fully filled. This uses custom irq handler + cond to wake up RX thread as it results in low and
	 * predictable latency. Using standard interface exposed from `imx6ull-sdma` would require additional
	 * communication between multiple threads and blocking on msgSend. This would increase risk of additional delays
	 * make it harder to reason about worst case scenario.
	 */
	interrupt(IRQ_SDMA, handleIrq, ctx, ctx->rxCond, 0);

	const char *files[] = { rxChannel, txChannel };
...
void dma_enable(void)
{
	...
	ctx->enabled = 1;
	static uint8_t handleStack[_PAGE_SIZE] __attribute__((aligned));
...
int dma_init(size_t size, size_t count, const char *rxChannel, const char *txChannel, bool (*rxCb)(const uint8_t *data, size_t size))
{
	struct sdmaCtx *const ctx = &sdma_common;

	if (mutexCreate(&ctx->rxLock) < 0) {
		log_error("mutexCreate failed");
		return -EIO;
	}
	if (condCreate(&ctx->rxCond) < 0) {
		log_error("condCreate failed");
		return -EIO;
	}
	ctx->rxCb = rxCb;

	if (sdmaInit(ctx, size, count, rxChannel, txChannel) < 0) {
		sdmaFree(ctx);
