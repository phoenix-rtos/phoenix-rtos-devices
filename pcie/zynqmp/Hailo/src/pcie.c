...
#include "../phoenix/phoenix_sema.h"
#include "../phoenix/phoenix_log.h"
#include "../phoenix/phoenix_mem.h"
#include "../phoenix/phoenix_wrappers.h"
#include "../phoenix/phoenix_completion.h"
#include "../phoenix/phoenix_pci_regs_compatibility.h"
...
/* Force allocation from driver - easier since no driver-to-user mappings */
static int force_allocation_from_driver = HAILO_FORCE_BUFFER_FROM_DRIVER;
...
#define HAILO_IRQ_FLAGS 0

/* Both of those as they might stay global have to be initialized at runtime */
...
static struct list_head g_hailo_board_list;
static struct semaphore g_hailo_add_board_mutex;
...
int hailo_bar_iomap(struct pci_dev *pdev, int bar, struct hailo_resource *resource)
{
...
int pcie_resources_init(struct pci_dev *pdev, struct hailo_pcie_resources *resources,
...
int hailo_pcie_probe(struct pci_dev *pDev)
{
	INIT_LIST_HEAD(&g_hailo_board_list);
	__SEMAPHORE_INITIALIZER(g_hailo_add_board_mutex, 1);
	...
	err = pcie_resources_init(pDev, &pBoard->pcie_resources, HAILO_BOARD_TYPE_HAILO8);
	if (err < 0) {
