...
void clbc_dataInStage(uint8_t epNum)
{
	...
	// if (epNum == 0) {
	// 	usb_endpointtypedef_t *pep = &clbc_common.dc->epIn[0];

	// 	if (clbc_common.dc->ep0State == USBD_EP0_DATA_IN) {

	// 		if (pep->rem_length > pep->maxpacket) {
	// 			pep->rem_length -= pep->maxpacket;

	// 			clbc_epTransmit(epNum, clbc_common.data->endpts[0], pep->rem_length);
	...
	// 				(pep->total_length < clbc_common.dc->ep0DataLen))
	// 			{
	// 				clbc_epTransmit(epNum, NULL, 0U);
	// 				clbc_common.dc->ep0DataLen = 0U;
	...
	// 				 * TODO: RADEK
	// 				 */
	// 				// if (clbc_common.dc->deviceState == USBD_STATE_CONFIGURED)
	// 				// {
...
void clbc_rxFifoData(int debug)
{
	...
	switch (pktstsVal) {
		...
		case 6:
			...
			if (debug == 1) {
				volatile uint8_t nazwa1 = clbc_common.dc->setup.bRequest;
				volatile uint8_t nazwa2 = clbc_common.dc->setup.bmRequestType;
				volatile uint16_t nazwa3 = clbc_common.dc->setup.wValue;
				__asm__ volatile("1: b 1b");
			}
			break;
