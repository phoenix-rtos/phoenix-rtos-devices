...
static int si5345_setReg(const si5345_revd_register_t *const reg)
{
	...
	if (ret != 0) {
		fprintf(stderr, "si5345: fail to read page address\n");
		return ret;
	}
	...
	if (current_page != desired_page) {
		...
		if (ret != 0) {
			fprintf(stderr, "si5345: fail to write page address\n");
			return ret;
		}
	}
	...
	if (ret != 0) {
		fprintf(stderr, "si5345: fail to write reg value, error: %i\n", ret);
	}
...
static int si5345_upload_config(void)
{
	...
	for (uint32_t i = 0; i < start_len; i++) {
		...
		if (ret != 0) {
			fprintf(stderr, "si5345: failed to upload register\n");
			return ret;
		}
	}
	// usleep(300 * 1000);
...
int si5345_statusWait(void)
{
	...
	/* Read some registers */
	ret |= i2c_regRead(0x69, 0x0C, &valC, 1);
	ret |= i2c_regRead(0x69, 0x0E, &valE, 1);
	ret |= i2c_regRead(0x69, 0x0D, &valD, 1);
	ret |= i2c_regRead(0x69, 0x11, &val11, 1);
	...
	while (tmp == 1) {
		...
		usleep(256);
		ret |= i2c_regRead(0x69, 0x0C, &valC, 1);
		ret |= i2c_regRead(0x69, 0x0F, &valF, 1);
		tmp = ((((valF) & (0x10)) >> 5) | ((valC) & (0x01)));
		if ((cnt % 0x100U) == 0) {
			printf("si5345: status 0xC:0x%x, 0xE:0x%x, 0xD:0x%x, 0x11:%x, 0xF:%x (...waiting for calibration...%i us).\r",
					valC, valE, valD, val11, valF, cnt);
		}

		if (cnt >= TIME_CHECK_PLL_CONFIG_US) {
			printf("si5345: 0xC:0x%x, 0xE:0x%x, 0xD:0x%x, 0x11:%x, 0xF:%x (...calibration not finished after %i us...exit...).\r\n",
					valC, valE, valD, val11, valF, cnt);
			printf("si5345: 0xC:0x%x(cal bit0:%i) and 0xF:%x(cal bit5:%i) will be checked one time again after %i us\r\n",
					valC, ((valC) & (0x01)), valF, tmp, DELAY_AFTER_PLL_CONFIG_US);
			tmp = 0;
		}
	}

	usleep(DELAY_AFTER_PLL_CONFIG_US);
	...
	printf("si5345: PLL status register 0xC:0x%x, 0xE:0x%x, 0xD:0x%x, 0x11:%x, 0xF:%x.\r\n",
...
int si5345_initPcieClk(void)
{
	...
	if (ret != 0) {
		fprintf(stderr, "si5345: fail to config i2c multiplexer: %i\n", ret);
		return ret;
	}
	...
	if (ret != 0) {
		fprintf(stderr, "si5345: fail to read si5345 whoami: %i\n", ret);
		return ret;
	}

	if ((chip_id[0] == 0x45) && (chip_id[1] == 0x53)) {
		printf("si5345: chip id valid 0x%x 0x%x 0x%x 0x%x \n",
				chip_id[0], chip_id[1], chip_id[2], chip_id[3]);
	}
	else {
		fprintf(stderr, "si5345: chip id invalid 0x%x 0x%x 0x%x 0x%x \n",
				chip_id[0], chip_id[1], chip_id[2], chip_id[3]);
	}

	/* Configure SI5345 */
	ret = si5345_upload_config();
	if (ret != 0) {
		fprintf(stderr, "si5345: fail to upload config: %i\n", ret);
		return ret;
	}

	return si5345_statusWait();
}
...
/* Configure MIO31 pin used for PCI Express endpoint reset as GPIO output */
static int si5345_configPcieReset(void)
{
	...
	writeRegMsk(si5345.gpio, 0x244, 0x20, 0x20);
	/* Set MIO31 output enable */
	writeRegMsk(si5345.gpio, 0x248, 0x20, 0x20);

	return ret;
}

static void si5345_assertPcieReset(void)
{
	/* Set MIO31 to LOW */
	writeRegMsk(si5345.gpio, 0x44, 0x20, 0x00);
}

static void si5345_deassertPcieReset(void)
{
	/* Set MIO31 to HIGH */
	writeRegMsk(si5345.gpio, 0x44, 0x20, 0x20);
}

int main(int argc, char **argv)
{
	/* Map GPIO register set */
	si5345.gpio = mmap(NULL, GPIO_REG_SET_SIZE, PROT_WRITE | PROT_READ,
			MAP_DEVICE | MAP_PHYSMEM | MAP_ANONYMOUS, -1, GPIO_REG_SET_ADDR);
	if (NULL == si5345.gpio) {
		fprintf(stderr, "si5345: fail to map GPIO registers memory\n");
		return EXIT_FAILURE;
	}

	/* Initialize PCI Express reset pin */
	if (si5345_configPcieReset() < 0) {
		fprintf(stderr, "si5345: fail to init reset pin\n");
		munmap((void *)si5345.gpio, GPIO_REG_SET_SIZE);
		return EXIT_FAILURE;
	}

	/* Assert PCI Express reset */
	si5345_assertPcieReset();

	if (si5345_initPcieClk() != 0) {
		fprintf(stderr, "si5345: failed to configure chip\n");
		munmap((void *)si5345.gpio, GPIO_REG_SET_SIZE);
		return EXIT_FAILURE;
	}
	else {
		printf("si5345: config success\n");
	}

	/* Deassert PCIE EXpress reset */
	si5345_deassertPcieReset();
	...
	oid_t oid;
	char *path = "/dev/si5345";
	if (create_dev(&oid, path) < 0) {
		fprintf(stderr, "si5345: cannot create device file\n");
		return EXIT_FAILURE;
	}
