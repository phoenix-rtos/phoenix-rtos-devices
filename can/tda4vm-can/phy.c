...
int mcan_writeRAM(struct mcu_mcan_module *mcan, struct mcan_txMessage *txMsg)
{
	...
	if (txFull == 1U || (txPutId > RAM_TX_QUEUE_ELEM_NUM)) {
		return -EBUSY;
	}
...
void mcan_readRxFIFO(struct mcu_mcan_module *mcan, uint8_t rxFifoID)
{
	...
	while (mcan->rx.xferSize > mcan->rx.xferCount) {
		...
		if (rxFrame->dlc > MCAN_DATA_FIELD_LENGTH) {
			rxFrame->dlc = MCAN_DATA_FIELD_LENGTH;
		}
...
int mcan_phyInit(struct mcu_mcan_module *mcan)
{
	...
	*(mcan->baseCore + cccr) &= ~(1UL << MCAN_CCCR_INIT);
	...
	if (mcan->canMode != MCU_MCAN_MODE_LOOPBACK && mcan->idx == MCU_MCAN_MODULE0) {
