...
#define EHCI_PERIODIC_SIZE         256
#define EHCI_PERIODIC_SIZE_DEFAULT 1024
...
static int ehci_init(hcd_t *hcd)
{
	...
	// TODO
	// if (((*hcd->base + hccparams) & (1 << 1)) == 0) {
	// 	/* Frame list length not programmable */
	// 	periodic_list_size = EHCI_PERIODIC_SIZE_DEFAULT;
	// }
	// else {
	// 	periodic_list_size = EHCI_PERIODIC_SIZE;
	// }

	for (i = 0; i < EHCI_PERIODIC_SIZE_DEFAULT; ++i)
		ehci->periodicList[i] = QH_PTR_INVALID;
	printf("%d\n", __LINE__);
