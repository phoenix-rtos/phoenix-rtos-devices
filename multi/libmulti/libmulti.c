...
#define LIBMULTI_HAS_GPIO_DRV 1
#endif
...
#define N_DRVS ((LIBMULTI_HAS_GPIO_DRV != 0 ? 1 : 0) + \
		(LIBMULTI_HAS_I2C_DRV != 0 ? 1 : 0) + \
		(LIBMULTI_HAS_SPI_DRV != 0 ? 1 : 0) + \
...
static int libmulti_handleGpio(multi_msg_t *multi)
{
	switch (multi->gpio.op) {
		case gpio_opGetPort:
			return libmulti_gpioGetPort(multi->gpio.port, &multi->gpio.val);

		case gpio_opSetPort:
			return libmulti_gpioSetPort(multi->gpio.port, multi->gpio.mask, multi->gpio.val);

		case gpio_opGetDir:
			return libmulti_gpioGetDir(multi->gpio.port, &multi->gpio.val);

		case gpio_opSetDir:
			return libmulti_gpioSetDir(multi->gpio.port, multi->gpio.mask, multi->gpio.val);
