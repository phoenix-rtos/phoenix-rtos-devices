...
#include "../phoenix/phoenix_firmware.h"
...
static const struct hailo_board_compatibility compat[HAILO_BOARD_TYPE_COUNT] = {
	[HAILO_BOARD_TYPE_HAILO8] = {
		...
		.stages = {
			{ .batch = hailo8_files_stg1, .trigger_address = 0xE0980, .timeout = FIRMWARE_WAIT_TIMEOUT_MS, .amount_of_files_in_stage = 3 },
		},
	},
	[HAILO_BOARD_TYPE_HAILO10H_LEGACY] = {
		...
		.stages = {
			{ .batch = hailo10h_legacy_files_stg1, .trigger_address = 0x88c98, .timeout = FIRMWARE_WAIT_TIMEOUT_MS, .amount_of_files_in_stage = 1 },
		},
	},
	[HAILO_BOARD_TYPE_HAILO10H] = {
		...
		.stages = {
			{ .batch = hailo10h_files_stg1, .trigger_address = 0x88c98, .timeout = FIRMWARE_WAIT_TIMEOUT_MS, .amount_of_files_in_stage = 3 },
			{ .batch = hailo10h_family_files_stg2, .trigger_address = 0x84000000, .timeout = PCI_EP_WAIT_TIMEOUT_MS, .amount_of_files_in_stage = 4 },
		},
	},
	...
	[HAILO_BOARD_TYPE_HAILO15L] = {
		...
		.stages = {
			{ .batch = hailo15l_files_stg1, .trigger_address = 0x88c98, .timeout = FIRMWARE_WAIT_TIMEOUT_MS, .amount_of_files_in_stage = 1 },
		},
	},
	[HAILO_BOARD_TYPE_MARS] = {
		...
		.stages = {
			{ .batch = hailo10h2_files_stg1, .trigger_address = 0x98c98, .timeout = FIRMWARE_WAIT_TIMEOUT_MS, .amount_of_files_in_stage = 3 },
			{ .batch = hailo10h_family_files_stg2, .trigger_address = 0x84000000, .timeout = PCI_EP_WAIT_TIMEOUT_MS, .amount_of_files_in_stage = 4 },
		},
	},
};
...
int hailo_pcie_write_firmware_batch(struct device *dev, struct hailo_pcie_resources *resources, u32 stage)
{
	...
	for (file_index = 0; file_index < amount_of_files; file_index++) {
		...
		if (!files_batch[file_index].is_mandatory) {
			dev_notice(dev, "Skipping non mandatory file %s\n", files_batch[file_index].filename);
			continue;
		}
