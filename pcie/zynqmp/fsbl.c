...
#define SERDES_L2_TM_PLL_DIG_37_OFFSET          0xA094
...
#define SERDES_L3_PLL_FBDIV_FRAC_3_MSB_OFFSET   0xE360
#define SERDES_L3_TM_DIG_6_OFFSET               0xD06C
#define SERDES_L3_TX_DIG_TM_61_OFFSET           0xC0F4
#define SERDES_L0_TM_AUX_0_OFFSET               0x10CC
#define SERDES_L2_TM_AUX_0_OFFSET               0x90CC
...
#define SERDES_L3_TM_MISC2_OFFSET               0xD89C
#define SERDES_L3_TM_IQ_ILL1_OFFSET             0xD8F8
#define SERDES_L3_TM_IQ_ILL2_OFFSET             0xD8FC
#define SERDES_L3_TM_ILL12_OFFSET               0xD990
#define SERDES_L3_TM_E_ILL1_OFFSET              0xD924
#define SERDES_L3_TM_E_ILL2_OFFSET              0xD928
#define SERDES_L3_TM_ILL11_OFFSET               0xD98C
#define SERDES_L3_TM_IQ_ILL3_OFFSET             0xD900
#define SERDES_L3_TM_E_ILL3_OFFSET              0xD92C
#define SERDES_L3_TM_ILL8_OFFSET                0xD980
#define SERDES_L3_TM_IQ_ILL8_OFFSET             0xD914
#define SERDES_L3_TM_IQ_ILL9_OFFSET             0xD918
#define SERDES_L3_TM_E_ILL8_OFFSET              0xD940
#define SERDES_L3_TM_E_ILL9_OFFSET              0xD944
...
#define SERDES_ICM_CFG0_OFFSET                  0x10010
#define SERDES_ICM_CFG1_OFFSET                  0x10014
#define SERDES_L1_TXPMD_TM_45_OFFSET            0x4CB4
#define SERDES_L1_TX_ANA_TM_118_OFFSET          0x41D8
#define SERDES_L3_TX_ANA_TM_118_OFFSET          0xC1D8
#define SERDES_L3_TM_CDR5_OFFSET                0xDC14
#define SERDES_L3_TM_CDR16_OFFSET               0xDC40
#define SERDES_L3_TM_EQ0_OFFSET                 0xD94C
#define SERDES_L3_TM_EQ1_OFFSET                 0xD950
#define SERDES_L1_TXPMD_TM_48_OFFSET            0x4CC0
#define SERDES_L1_TX_ANA_TM_18_OFFSET           0x4048
...
static int maskPoll(uint32_t *base, uint32_t offset, uint32_t mask)
{
	int i = 0;
...
static uint32_t maskRead(uint32_t *base, uint32_t offset, uint32_t mask)
{
	return (readReg(base, offset) & mask);
}

int serdesFixcalCode(uint32_t *base)
{
	int mask_status = 1;
	...
	do {
		...
		mask_status = maskPoll(base, 0xEF14, 0x2);
		if (mask_status == 0) {
			/*failure here is because of calibration done timeout*/
			printf("fsbl: fail, SERDES initialization timed out\n");
			return mask_status;
		}

		p_code = maskRead(base, 0xEF18, 0xFFFFFFFF);    /*PMOS code*/
		n_code = maskRead(base, 0xEF1C, 0xFFFFFFFF);    /*NMOS code*/
		/*m_code = mask_read(0xFD40EF20, 0xFFFFFFFF)*/; /*MPHY code*/
		i_code = maskRead(base, 0xEF24, 0xFFFFFFFF);    /*ICAL code*/
		r_code = maskRead(base, 0xEF28, 0xFFFFFFFF);    /*RX code*/
	...
	L3_TM_CALIB_DIG20 = maskRead(base, 0xEC50, 0xFFFFFFF0); /*read DIG20*/
	...
	L3_TM_CALIB_DIG19 = maskRead(base, 0xEC4C, 0xFFFFFF18); /*read DIG19*/
	...
	L3_TM_CALIB_DIG18 = maskRead(base, 0xEC48, 0xFFFFFF0F); /*read DIG18*/
	L3_TM_CALIB_DIG18 = L3_TM_CALIB_DIG18 | ((n_code & 0x7) << 5) | 0x10;


	/*L3_TM_CALIB_DIG16[2:0]	RX Code [3:1]*/
	L3_TM_CALIB_DIG16 = maskRead(base, 0xEC40, 0xFFFFFFF8); /*read DIG16*/
	...
	L3_TM_CALIB_DIG15 = maskRead(base, 0xEC3C, 0xFFFFFF30); /*read DIG15*/
	...
	L3_TM_CALIB_DIG14 = maskRead(base, 0xEC38, 0xFFFFFF3F); /*read DIG14*/
...
static int serdes_rst_seq(uint32_t *base, uint32_t pllsel, uint32_t lane3_protocol, uint32_t lane3_rate, uint32_t lane2_protocol, uint32_t lane2_rate, uint32_t lane1_protocol, uint32_t lane1_rate, uint32_t lane0_protocol, uint32_t lane0_rate)
{
	...
	if (pllsel == 0) {
		int wait_ret = maskPoll(base, SERDES_L0_PLL_STATUS_READ_1, 0x00000010U);
		if (wait_ret == 0) {
			printf("fsbl: timeout on PS GTR 0 ready wait\n");
		}
	}
	if (pllsel == 1) {
		int wait_ret = maskPoll(base, SERDES_L1_PLL_STATUS_READ_1, 0x00000010U);
		if (wait_ret == 0) {
			printf("fsbl: timeout on PS GTR 1 ready wait\n");
		}
	}
	if (pllsel == 2) {
		int wait_ret = maskPoll(base, SERDES_L2_PLL_STATUS_READ_1, 0x00000010U);
		if (wait_ret == 0) {
			printf("fsbl: timeout on PS GTR 2 ready wait\n");
		}
	}
	if (pllsel == 3) {
		int wait_ret = maskPoll(base, SERDES_L3_PLL_STATUS_READ_1, 0x00000010U);
		if (wait_ret == 0) {
			printf("fsbl: timeout on PS GTR 3 ready wait\n");
		}
	}
	usleep(50);
...
// usb = 0; sgmii = 0; DP = 0;
{
	...
	if (lane0_protocol == 1) {
		if (lane0_rate == 0) {
			...
			printf("fsbl: illcalib PCIE first\n");
			serdes_illcalib_pcie_gen1(base, 0, lane3_protocol, lane3_rate, lane2_protocol, lane2_rate, lane1_protocol, lane1_rate, lane0_protocol, 0, 0);
			printf("fsbl: illcalib PCIE second\n");
...
unsigned long psuSerdesInitData(uint32_t *base)
{
	/*
	...
	*/
	writeRegMsk(base, SERDES_PLL_REF_SEL1_OFFSET, 0x0000001FU, 0x00000009U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_PLL_REF_SEL2_OFFSET, 0x0000001FU, 0x00000008U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_PLL_REF_SEL3_OFFSET, 0x0000001FU, 0x0000000FU);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L2_TM_PLL_DIG_37_OFFSET,
	...
	*/
	writeRegMsk(base, SERDES_L2_TM_DIG_6_OFFSET, 0x00000003U, 0x00000003U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_PLL_FBDIV_FRAC_3_MSB_OFFSET,
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_DIG_6_OFFSET, 0x0000000FU, 0x0000000FU);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TX_DIG_TM_61_OFFSET,
	...
	*/
	writeRegMsk(base, SERDES_L2_TM_AUX_0_OFFSET, 0x00000020U, 0x00000020U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L2_TM_ILL12_OFFSET, 0x000000FFU, 0x00000010U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L2_TM_E_ILL1_OFFSET, 0x000000FFU, 0x000000FEU);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L2_TM_E_ILL2_OFFSET, 0x000000FFU, 0x00000000U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L2_TM_E_ILL3_OFFSET, 0x000000FFU, 0x00000000U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_MISC2_OFFSET, 0x00000080U, 0x00000080U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_IQ_ILL1_OFFSET,
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_IQ_ILL2_OFFSET,
			0x000000FFU, 0x0000007DU);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_ILL12_OFFSET, 0x000000FFU, 0x00000001U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_E_ILL1_OFFSET, 0x000000FFU, 0x0000009CU);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_E_ILL2_OFFSET, 0x000000FFU, 0x00000039U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_ILL11_OFFSET, 0x000000F0U, 0x00000020U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_IQ_ILL3_OFFSET,
			0x000000FFU, 0x0000007DU);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_E_ILL3_OFFSET, 0x000000FFU, 0x00000064U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_ILL8_OFFSET, 0x000000FFU, 0x000000FFU);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_IQ_ILL8_OFFSET,
			0x000000FFU, 0x000000F7U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_IQ_ILL9_OFFSET,
			0x00000001U, 0x00000001U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_E_ILL8_OFFSET, 0x000000FFU, 0x000000F7U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_E_ILL9_OFFSET, 0x00000001U, 0x00000001U);
	/*##################################################################### */
	...
	*/
	...
	if (NULL == base) {
		printf("fsbl: fail to map SIOU registers memory\n");
	}
	...
	*/
	writeRegMsk(base, SERDES_ICM_CFG0_OFFSET, 0x00000077U, 0x00000041U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_ICM_CFG1_OFFSET, 0x00000077U, 0x00000023U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L1_TXPMD_TM_45_OFFSET,
	...
	*/
	writeRegMsk(base, SERDES_L1_TX_ANA_TM_118_OFFSET,
			0x00000001U, 0x00000001U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_CDR5_OFFSET, 0x000000FFU, 0x000000E6U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_CDR16_OFFSET, 0x0000001FU, 0x0000000CU);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_EQ0_OFFSET, 0x00000020U, 0x00000020U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L3_TM_EQ1_OFFSET, 0x00000007U, 0x00000006U);
	/*##################################################################### */
	...
	*/
	writeRegMsk(base, SERDES_L1_TXPMD_TM_48_OFFSET,
	...
	*/
	writeRegMsk(base, SERDES_L1_TX_ANA_TM_18_OFFSET,
...
static int pcie_deassertPartOfPcieResets(void)
{
	int ret = 0;
...
void fill_pcieAttributes(uint32_t *pcireg, uint32_t *serdes)
{
	/*
	...
	*/
	writeRegMsk(pcireg, PCIE_ATTRIB_ATTR_79_OFFSET, 0x00000020U, 0x00000020U);
	/*##################################################################### */
...
static int pcie_deassertPcieCtrl(void)
{
	platformctl_t ctl3 = {
		.action = pctl_set,
		.type = pctl_devreset,
...
int main(int argc, char **argv)
{
	...
	if (NULL == serdes) {
		printf("fsbl: fail to map SERDES registers memory\n");
		return -1;
	}
	uint32_t *pcireg = mmap(NULL, PCIREG_SIZE,
			...
			PCIREG_ADDRESS);
	if (NULL == pcireg) {
		printf("fsbl: fail to map PCIE registers memory\n");
		return -1;
	}

	printf("fsbl: init PCIE clock\n");
	ret = pcie_initClock();
	if (ret != 0) {
		printf("fsbl: fail to config clock\n");
		return ret;
	}
	...
	printf("fsbl: assert reset on all 3 PCIE peripherals\n");
	...
	printf("fsbl: fix calibration of SERDES peripheral\n");
	int retNegated = serdesFixcalCode(serdes);
	if (retNegated != 1) {
		ret = -1;
		printf("fsbl: fail to fix calibration of SERDES peripheral\n");
		return ret;
	}

	printf("fsbl: init SERDES\n");
	(void)psuSerdesInitData(serdes);

	printf("fsbl: deassert PCIE CFG and PCIE BRIDGE reset\n");
	ret = pcie_deassertPartOfPcieResets();
	if (ret != 0) {
		printf("fsbl: fail to deassert PCIE CFG and PCIE BRIDGE reset\n");
		return ret;
	}

	printf("fsbl: config PCIE attributes\n");
	fill_pcieAttributes(pcireg, serdes);

	printf("fsbl: deassert PCIE CTRL reset\n");
	ret = pcie_deassertPcieCtrl();
	if (ret != 0) {
		printf("fsbl: fail to deassert PCIE CTRL reset\n");
		return ret;
	}

	/* Wait for PHY ready */
	int wait_ret = maskPoll(serdes, SERDES_L0_PLL_STATUS_READ_1_OFFSET, 0x00000010U);
	if (wait_ret == 0) {
		printf("fsbl: timeout on PS GTR 0 ready wait\n");
	}
