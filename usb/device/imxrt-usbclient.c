...
static int ctrlqh_init(void)
{
    ...
    /* map queue head list */
    dc.endptqh = (void*) (OCRAM2_BASE_ADRR + USB_SIZE * counter_OCRAM2++);

    if (dc.endptqh == MAP_FAILED)
        return -ENOMEM;
\
    memset((void *)dc.endptqh, 0, USB_SIZE);

    qh_addr = (u32)dc.endptqh;
...
static int dtd_init(int endpt)
{
    ...
    buff = (void*) (OCRAM2_BASE_ADRR + USB_SIZE * counter_OCRAM2++);

    if (buff == MAP_FAILED)
        return -ENOMEM;

    memset(buff, 0, USB_SIZE);
...
int usbclient_init(usbclient_conf_t *conf)
{
    ...
    usb_data.read_buffer.data = (void* )(OCRAM2_BASE_ADRR + USB_SIZE * counter_OCRAM2++);
    usb_data.pread_buffer = (u32) usb_data.read_buffer.data;
    usb_data.write_buffer.data = (void*) (OCRAM2_BASE_ADRR + USB_SIZE * counter_OCRAM2++);
    usb_data.pwrite_buffer = (u32) usb_data.write_buffer.data;

    usb_data.local_conf = (void* )(OCRAM2_BASE_ADRR + USB_SIZE * counter_OCRAM2++);
...
int usbclient_destroy(void)
{
    ...
    *(dc.base + usbcmd) &= ~1;
    ...
    munmap(usb_data.local_conf, USB_SIZE);
    munmap((void *)dc.base, USB_SIZE);
    munmap((void *)((u32)dc.endptqh[2].head), USB_SIZE);
    munmap((void *)dc.endptqh, USB_SIZE);
