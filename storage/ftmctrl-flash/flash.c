...
static struct {
	...
	size_t nmodels;
	const cfi_dev_t *devs[FLASH_DEVICES];
} common;
...
static const cfi_dev_t *flash_query(cfi_info_t *cfi)
{
	for (size_t i = 0; i < common.nmodels; i++) {
		const addr_t queryOffs = 0x0;
		const cfi_dev_t *dev = common.devs[i];
...
int flash_init(struct _storage_devCtx_t *ctx)
{
	...
	const cfi_dev_t *dev = flash_query(&ctx->cfi);
