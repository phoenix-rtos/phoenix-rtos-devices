...
static int readRegs(struct ade9113_ctx *ctx, uint8_t reg, uint8_t *values, uint8_t *valuesNext, uint8_t size)
{
	if (size != 4) {
		return -1;
	}
...
{
	return readRegs(ctx, reg, values, NULL, 4);
}
...
int ade9113_readRegsU16(struct ade9113_ctx *ctx, uint8_t reg, uint16_t *values, uint8_t size)
{
	...
	uint8_t high[4];
	uint8_t low[4];
	if (readRegs(ctx, reg, high, low, 4) < 0) {
		return -1;
	}
	for (int i = 0; i < 4; ++i) {
		values[i] = (high[i] << 8) | low[i];
	}
	return 0;
}
