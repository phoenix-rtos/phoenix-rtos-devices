...
typedef struct {
	...
	const struct libdma_per *dma_multiChn;
#endif
} pwm_tim_data_t;
...
#if USE_DSHOT_DMA
static int pwm_initDMA(pwm_tim_id_t timer, pwm_ch_id_t chn, bool doMultichannel, const struct libdma_per **target)
{
	...
	int res = libxpdma_acquirePeripheral(dma_tim_upd, timer, 0, target);
	if ((res < 0) || (*target == NULL)) {
		return res;
	}

	volatile uint32_t *destination = doMultichannel ?
			(pwm_setup.timer[timer].base + tim_dmar) :
			(pwm_setup.timer[timer].base + PWM_CCR_REG(chn));
	res = (res < 0) ? res : libxpdma_configureChannel(*target, dma_mem2per, dma_priorityVeryHigh, NULL);
	...
	res = (res < 0) ? res : libxpdma_configurePeripheral(*target, dma_mem2per, &cfg);
	return res;
}

static int pwm_initDMAChannel(pwm_tim_id_t timer, pwm_ch_id_t chn)
{
	return pwm_initDMA(timer, chn, false, &pwm_common.timer[timer].dma_per[chn]);
}

static int pwm_initDMAMultiChannel(pwm_tim_id_t timer)
{
	return pwm_initDMA(timer, 0, true, &pwm_common.timer[timer].dma_multiChn);
}
#endif
...
int pwm_setBitSequence(pwm_tim_id_t timer, pwm_ch_id_t chn, void *data, uint32_t nbits, uint8_t datasize, int flags)
{
	...
	res = (res < 0) ? res : pwm_set(timer, chn, pwm_idleValue);
	volatile int done = 0;
...
int pwm_setBitSequence4(pwm_tim_id_t timer, const uint16_t chnRaw[4], const uint16_t val16[4], uint16_t hcmp, uint16_t lcmp, int flags)
{
	...
	int valIdx[PWM_CHN_NUM];
	memset(valIdx, -1, sizeof(valIdx));
	(void)flags;
	...
	res = pwm_initDMAMultiChannel(timer);
	if (res < 0) {
	...
	const struct libdma_per *per = t->dma_multiChn;
	...
	for (int i = pwm_ch1; i <= pwm_ch4; i++) {
		res = pwm_set(timer, i, pwm_idleValue);
		if (res < 0) {
			mutexUnlock(t->dmaMutex);
			return res;
		}
