...
#include <hal/if.h>
#include <hal/system_MVF50GS10MK50.h>
#include <proc/if.h>
...
typedef struct MmcblkIoOps_t{
	...
	int (*sendCommand)(void *card, u32 cmd, u32 cmd_arg, s32 block_num, u16 block_size, void *admaDT);
	MmcblkResponse_t (*waitForResponse)(void *card, int cmd);
	int (*transferWait)(void *card);
	int (*waitBusy)(void *card);
	void * (*setupDMA)(void *card, void *bufptr, s32 bufsize, FreePtr **fp, char *htBuf);
	void (*freeDMA)(FreePtr *fp);
...
typedef struct MmcblkCardOps_t{
	...
	int (*write)(MmcblkCard_t *card, offs_t offs, char *buff, unsigned int len);
	int (*read)(MmcblkCard_t *card, offs_t offs, char *buff, unsigned int len);
...
typedef struct MmcblkDesc_t{
	u32 pnum;
	MmcblkPortDesc_t pdesc[];
} MmcblkDesc_t;
...
extern MmcblkDesc_t *bsp_mmcblkInitParams(void);


//functions equivalent to those from dev/ata/ata_generic.h
//int MMCBLK_polling(struct ata_channel *ac, u8 advanced_check);
//int MMCBLK_print_error(struct ata_dev *ad, u8 err);
//int MMCBLK_access(u8 direction, struct ata_dev *ad, u32 lba,u8 numsects, void *buffer);
//int MMCBLK_read_sectors(struct ata_dev *ad, u8 numsects, u32 lba, void *buff);
//int MMCBLK_write_sectors(struct ata_dev *ad, u8 numsects, u32 lba, void *buff);
