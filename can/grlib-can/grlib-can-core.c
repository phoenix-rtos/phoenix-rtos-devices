...
int __attribute__((section(".interrupt"), aligned(0x1000))) grlibCan_irqHandler(unsigned int irqNum, void *arg)
{
	...
	if ((device->penIsr & GRLIB_CAN_RX_IRQ) != 0) {
		ret = 1;
		dev->device->irqClrReg |= GRLIB_CAN_RX_IRQ;
	}

	if ((device->penIsr & GRLIB_CAN_TX_IRQ) != 0) {
		dev->txBufStatus = txBufReady;
		ret = 1;
	}

	if ((device->penIsr & GRLIB_CAN_TXEMPTY_IRQ) != 0) {
		ret = 1;
		dev->txStatus = txDone;
	}

	if ((device->penIsr & (GRLIB_CAN_BMRDERR_IRQ | GRLIB_CAN_BMWRERR_IRQ | GRLIB_CAN_OR_IRQ | GRLIB_CAN_BUSSOFF_IRQ | GRLIB_CAN_PASS_IRQ)) != 0) {
		...
		const uint32_t length)
{
	...
	dev->device->irqMskSet |= GRLIB_CAN_TXEMPTY_IRQ;

	while (dev->device->txWrtPtr != dev->device->txRdPtr && dev->txStatus == txOngoing) {
		if (dev->deviceStatus == deviceError) {
			(void)mutexUnlock(dev->txLock);
			return -i;
		}
