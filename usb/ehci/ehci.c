...
static void ehci_qhConf(ehci_qh_t *qh, usb_pipe_t *pipe)
{
	...
	if (pipe->type == usb_transfer_interrupt) {
		printf("qhconf usb_transfer_interrupt\n");
		if (pipe->dev->speed == usb_high_speed) {
			printf("pipe->interval=%d\n", pipe->interval);
...
static void ehci_irqThread(void *arg)
{
	...
	for (;;) {
		...
		/* TODO: Think through how to guarantee race-free
		   status passing between irqHandler and irqThread.

		   The irqThread must clear the handler interrupts,
		...
		if (ehci->status & (USBSTS_UI | USBSTS_UEI)) {
			/* TODO propagate back UEI to usb stack */
...
static int ehci_init(hcd_t *hcd)
{
	...
	log_debug("base: %p, opbase: %p", (void *)ehci->base, (void *)ehci->opbase);
