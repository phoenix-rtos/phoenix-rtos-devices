...
#define ECC_BITFLIP_THRESHOLD 30  /* Min number of bitflips for page rewrite */
#define ECC0_BITFLIP_STRENGTH 40  /* ECC strength for metadata chunk */
#define ECCN_BITFLIP_STRENGTH 40  /* ECC strength for data chunk */
...
static inline int chunk_is_uncorrectable(unsigned int chunkidx, unsigned int flips)
{
	if (CHUNK_IS_META(chunkidx)) {
		return flips > ECC0_BITFLIP_STRENGTH;
	}
	else {
		return flips > ECCN_BITFLIP_STRENGTH;
	}
}
...
static int _flashmtd_chunkFlips(struct _storage_t *strg, flashdrv_meta_t *meta, uint32_t pageaddr, unsigned int chunkidx)
{
	...
	const size_t metalen = strg->dev->mtd->oobSize * CHAR_BIT + ECC_GF * ECC0_BITFLIP_STRENGTH;
	const size_t datalen = ECCN_DATA_SIZE * CHAR_BIT + ECC_GF * ECCN_BITFLIP_STRENGTH;
	void *raw = NULL;
	size_t boffs, blen;
	int ret;
	...
	switch (meta->errors[chunkidx]) {
		...
		case flash_uncorrectable:
			/* ECC reports chunk as uncorrectable in case of an erased page with bitflips within that chunk area */
...
/* Returns maximum number of bitflips or number smaller than 0 in case of error */
	...
	for (eb = startBlock; eb < endBlock; ++eb) {
		...
		if (res < 0) {
			...
			if (res < 0) {
				...
				return res; /* can't mark as badblock, unrecoverable error */
			}
		}
		else {
			++erased;
