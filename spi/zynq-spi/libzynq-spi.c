...
typedef struct {
	...
	int rst;                 /* Reset subsystem peripheral ID */
	unsigned int clk_dev_id; /* ID of peripheral in clock */
#endif
...
static spi_t devs[] = {
	{
		...
		.rst = pctl_devreset_lpd_spi0,        /* Reset number */
		.clk_dev_id = pctl_devclock_lpd_spi0, /* ID of peripheral in clock */
#endif
		.pins = {
	...
	},
	{
		...
		.rst = pctl_devreset_lpd_spi1,        /* Reset number */
		.clk_dev_id = pctl_devclock_lpd_spi1, /* ID of peripheral in clock */
#endif
		.pins = {
...
static int spi_initClk(spi_t *spi)
{
	...
	pctl.devclock.dev = spi->clk_dev_id;
	pctl.devclock.src = 0;
...
int spi_init(unsigned int dev)
{
	...
	if (err < 0) {
		fprintf(stderr, "libzynq-spi: failed to perform peripheral reset\n");
		return err;
	}
#else
	(void)spi_initPins;
