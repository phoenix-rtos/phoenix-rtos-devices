...
int main(int argc, char **argv)
{
	...
	if ((pid = fork()) < 0) {
		...
		sleep(10);
		fprintf(stderr, "virto-blk: failed to successfully initialize server\n");
		exit(EXIT_FAILURE);
	}

	/* Set child exit handler */
	signal(SIGUSR1, virtio_blk_signalexit);
	...
	if ((err = portCreate(&virtio_blk_common.port)) < 0) {
	...
	virtio_blk_msgloop(&virtio_blk_common.port);
