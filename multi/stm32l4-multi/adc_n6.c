...
/* This address is in BSEC peripheral, corresponds to OTP word #110.
 * It is marked as reserved in the documentation. */
/* TODO: read e-fuses in adc_init() if necessary */
static const unsigned short *const vrefint_calib_data = (void *)0x560091b8;
static const unsigned short vrefint_value = 1800;
...
static void adc_wakeup(int adc)
{
	...
	*(base + adc_cr) &= ~ADC_CR_DEEPPWD;
	...
	/* On STM32N6 ADC has no internal voltage regulator */

	/* Set to no analog watchdog, single conversion mode,
	 * 12-bit resolution (maximum supported), data stored to DR */
	*(base + adc_cfgr1) = 0;
...
{
	volatile unsigned int *bases[MAX_ADC];
	...
	do {
		...
		for (int i = adc1; i <= MAX_ADC; i++) {
			...
			allReady = allReady && ((*(bases[i] + adc_isr) & bit) != 0);
		}
	} while (!allReady);
...
unsigned short adc_conversion(int adc, char chan)
{
	...
	volatile unsigned int *base_vrefint = adc_common.base + adc_getOffs(adc);
	...
	if (adc == VREFINT_ADC) {
		*(base_vrefint + adc_pcsel) = (1 << VREFINT_CHANNEL) | (1 << chan);
		if (chan != VREFINT_CHANNEL) {
			*(base_vrefint + adc_sqr1) = (chan << 12) | (VREFINT_CHANNEL << 6) | 1;
			n_conversions = 2;
		}
		else {
	...
	vref = (unsigned short)results[0];
	out = (vrefint_value * (*vrefint_calib_data)) / vref;
	if (n_conversions > 1) {
		val = (unsigned short)results[1];
		out = (out * val) / ((1 << 12) - 1);
	}
...
int adc_init(void)
{
	...
	uint64_t freq;
	if (clockdef_getClock(clkid_hclk, &freq) < 0) {
		return -1;
	}

	/* Limit ADC frequency to under 100 MHz */
	unsigned int prescaler = 1;
	while (freq > 100 * 1000 * 1000) {
		freq /= 2;
		prescaler *= 2;
		if (prescaler > 256) {
			return -1;
