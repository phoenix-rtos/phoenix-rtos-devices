...
void clbc_sendEpData(uint8_t epNum)
{
	...
	while (((USB_REG(DTXFSTS0 + EP_STRIDE * epNum) & 0xFFFFUL) >= count32b) &&
			(ep->in.xfer_count < ep->in.xfer_size) && (len > 0UL)) {
		...
		for (cnt = 0; cnt < count32b; cnt++) {
			USB_REG(FIFO_BASE_OFF + epNum * (FIFO_SIZE / 4)) = UNALIGNED32_READ(src);
			src += sizeof(uint32_t);
		}
		ep->in.xfer_count += len;
		ep->in.xfer_buf += len;

		len = (uint16_t)ep->in.xfer_size - (uint16_t)ep->in.xfer_count;
		count32b = ((len + 3U) / 4U);
	}
